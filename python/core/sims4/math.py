...
# Compiled at: 2020-10-08 12:28:50
...
def circle_contains_position_2d(circle, position, return_dist_sq=False):
    ...
    return (
     dist_sq < pow(circle.radius, 2) if (not return_dist_sq) else (dist_sq < pow(circle.radius, 2)), dist_sq)
...
class Location(_Location):
    ...
    def almost_equal(self, other):
        ...
        if not vector3_almost_equal_2d(self.transform.translation, other.transform.translation):
            return False
        if not quaternion_almost_equal(self.transform.orientation, other.transform.orientation):
            return False
        return True

...
# Compiled at: 2017-04-05 17:29:43
...
class AutonomyService(Service):
    ...
    def _select_best_result(self, result_scores, autonomy_request, consider_all_options=False, randomization_override=DEFAULT, archive_if_enabled=True):
        ...
        if selected_interaction is not None:
            if selected_interaction.is_super:
                if not selected_interaction.use_best_scoring_aop:
    ...
    def choose_best_interaction(self, scored_interactions, autonomy_request, consider_all_options=False, randomization_override=DEFAULT, interaction_prefix=''):
        ...
        if randomization == autonomy.settings.AutonomyRandomization.ENABLED:
            ...
            if not gsi_handlers.autonomy_handlers.archiver.enabled or autonomy_request.gsi_data is not None:
                if top_options[-1].score > 0:
            ...
            if autonomy_request.consider_scores_of_zero and top_options[-1].score <= 0 or top_options[-1].score > 0:
        ...
        if chosen_scored_interaction_data is None:
            return
        if not autonomy_request.is_script_request:
            if autonomy_request.context.source == interactions.context.InteractionContext.SOURCE_AUTONOMY:
                if chosen_scored_interaction_data.interaction.is_super:
                    if multitasking_roll > chosen_scored_interaction_data.multitasking_percentage:
                        if autonomy_request.gsi_data is not None:
    ...
    def _recalculate_scores_based_on_route_time(self, scored_interactions):
        ...
        max_route_time = max(scored_interactions, key=(lambda scored_interaction_data: scored_interaction_data.route_time
)).route_time
        if max_route_time == 0:
            return scored_interactions
    ...
    def save_options(self, options_proto):
        ...
        if state_setting == autonomy.settings.AutonomyState.FULL:
            ...
            if state_setting == autonomy.settings.AutonomyState.LIMITED_ONLY:

...
# Compiled at: 2020-06-22 14:32:47
  ...
  default_hour=9), **kwargs)
    ...
    def _should_run_cached_interaction(self, interaction_to_run):
        ...
        if super_interaction is None or super_interaction.is_finishing:
            return False
        if super_interaction.phase_index is not None:
            if interaction_to_run.affordance not in super_interaction.all_affordances_gen(phase_index=(super_interaction.phase_index)):
                return False
    ...
    def _get_number_mixers_cache(self):
        additional_mixers_to_cache = sum((si.additional_mixers_to_cache() for si in self.owner.si_state if not si.is_finishing if  if not si.is_finishing))
    ...
    def update_sleep_schedule(self):
        ...
        for sleep_schedule_entry in sorted((sleep_schedule.schedule), key=(lambda entry: entry.time_from_work_start
), reverse=True):

...
class Career(HasTunableReference, careers.career_base.CareerBase, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.CAREER)):
 ...
 L.1446        66  LOAD_FAST                'stack'
               ...
               78  POP_TOP          
               80  JUMP_LOOP            62  'to 62'
               82  POP_BLOCK        
               84  JUMP_LOOP            18  'to 18'
...
Parse error at or near `JUMP_LOOP' instruction at offset 96
        ...
        for entitlement in Career.TRAIT_BASED_CAREER_LEVEL_ENTITLEMENTS:
            if sim_info.trait_tracker.has_trait(entitlement.trait):
                for career_level in entitlement.career_entitlements:
                    ...
                    if test_result:
                        if sim_info.career_tracker.get_career_by_uid(career_level.career.guid64) is None:
        ...
        for career_tuning in all_possible_careers:
            ...
            if test_result:
                if sim_info.career_tracker.get_career_by_uid(career_tuning.guid64) is None:
                    for new_level, _, current_track in career_tuning.get_career_entry_level(career_history=(sim_info.career_tracker.career_history),
                      resolver=(SingleSimResolver(sim_info))):
            ...
            if sim_info.is_selectable:
                if sim_info.valid_for_distribution:
                    ...
                    msg = Career.get_join_career_pb(sim_info, num_careers_to_show=num,
                      default_career_selection_data=default_career_selection_data)
                    Distributor.instance().add_op(sim_info, GenericProtocolBufferOp(Operation.SELECT_CAREER_UI, msg))
        else:
            if self.career_op == career_ops.CareerOps.QUIT_CAREER:
                if len(sim_info.career_tracker.get_quittable_careers()) == 1:
                    ...
                    for career in sim_info.career_tracker.get_quittable_careers().values():
                        ...
                        dialog = None
                        if career.quittable_data.tested_quit_dialog is not None:
                            for career_dialog_test in career.quittable_data.tested_quit_dialog:
                                if career_dialog_test.test_set.run_tests(resolver):
                                    dialog = career_dialog_test.quit_dialog(sim_info, resolver)
                                    break

                        if dialog is None:
                            dialog = career.quittable_data.quit_dialog(sim_info, resolver)
                        else:
                            dialog.show_dialog(on_response=on_quit_dialog_response, additional_tokens=(
                             career_name, job_title, career.get_company_name()))

                else:
                    if not sim_info.is_selectable or sim_info.valid_for_distribution:
                        msg = Career.get_quit_career_pb(sim_info)
                        Distributor.instance().add_op(sim_info, GenericProtocolBufferOp(Operation.SELECT_CAREER_UI, msg))
            else:
                if self.career_op == career_ops.CareerOps.CALLED_IN_SICK:

...
class CareerLevelOp(BaseLootOperation):
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        if not (career is None or career.can_change_level(demote=demote)):
            return
        if demote:
            career.demote()
        else:
    ...
    def _apply_to_career(self, sim_info, career, resolver):
        for op in self.operations:
            if op.operation_type == CareerLootOp.OP_PERFORMANCE:
                ...
                if op.operation_type == CareerLootOp.OP_MONEY or op.operation_type == CareerLootOp.OP_FINE:
                    ...
                    if op.operation_type == CareerLootOp.OP_RETIRE:
                        sim_info.career_tracker.retire_career(career.guid64)
                    else:
                        if op.operation_type == CareerLootOp.OP_PTO:
                            career.add_pto(op.amount)
                            career.resend_career_data()
                        else:
                            if op.operation_type == CareerLootOp.OP_TAKE_DAY_OFF:
                                career.request_day_off(op.reason)
                            else:
                                if op.operation_type == CareerLootOp.OP_PROMOTE:
                                    ...
                                    if op.operation_type == CareerLootOp.OP_DEMOTE:
                                        ...
                                        if op.operation_type == CareerLootOp.OP_FIRE:

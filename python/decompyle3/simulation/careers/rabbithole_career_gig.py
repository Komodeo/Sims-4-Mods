...
# Compiled at: 2021-04-22 13:28:26
...
class RabbitholeGig(Gig):
    ...
    def _determine_gig_outcome(self, **kwargs):
        ...
        if resolver(self.negative_mood_tuning.negative_mood_test):
            if random.random() <= self.negative_mood_tuning.failure_chance:
                self._gig_result = GigResult.FAILURE
                return
    ...
    def create_picker_row(cls, description=None, scheduled_time=None, owner=None, gig_customer=None, enabled=True, **kwargs):
        ...
        if not enabled or cls.disabled_tooltip is not None:

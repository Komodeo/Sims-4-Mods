...
# Compiled at: 2021-04-21 19:16:56
...
class BaseStatisticTracker(SimInfoTracker):
    ...
    def remove_statistics_on_travel(self):
        for statistic in tuple(self._statistics_values_gen()):
            if not statistic.persisted:
                ...
                if not self.owner is None or self.owner.is_statistic_type_added_by_modifier(stat_type):
    ...
    def get_statistic(self, stat_type, add=False):
        try:
            ...
            if self._statistics:
                stat = self._statistics.get(stat_type)
                if not stat is None or self.has_statistic(stat_type):
            ...
            if not must_end_recovery or stat_type in self._recovery_add_in_progress:
        ...
        for stat_type in list(self._statistics):
            ...
            if stat is not None:
                if ignore_ranked:
                    if stat.is_ranked:
                        continue
            if visible_only or core_only:
                if not (visible_only and stat.is_visible):
                    if core_only:
                        if stat.core:
                            pass
    ...
    def _should_add_commodity_from_gallery(self, statistic_type, skip_load):
        ...
        if self.owner.is_sim:
            if skip_load:
                if statistic_type.gallery_load_behavior != GalleryLoadBehavior.LOAD_ONLY_FOR_SIM:
                    return False
        else:
                ...
                if stat_to_test.instance_required:
                    ...
                    for listener in callback_listeners:
                        if listener.should_seed:
                            if not hasattr(listener._callback, '__self__') or listener._callback.__self__ is listener.stat:
                                listener.destroy()
                                continue
                            else:
                                listener.stat = None

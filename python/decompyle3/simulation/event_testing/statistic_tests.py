        ...
        for target in test_targets:
            if target is None:
                ...
                if self.ranked_stat is not None:
                    ...
                    if not (ranked_stat_inst is not None and self.ranked_stat.is_skill and ranked_stat_inst.is_initial_value) and self.test_against_highest_rank:
                ...
                if reserve_participants is not None:
                    for sim in reserve_participants:
                        ...
                        if sim_instance is not None:
                            if obj.may_reserve(sim_instance):
                                break
                    else:
                        continue
                    if self.test_aops or self.check_affordance_suppression:
                        ...
                        if self.test_connectivity_to_target:
                            ...
                            elif self.requirements == self.REQUIRE_ANY:
                                return TestResult.TRUE
            ...
            if self.only_other_sims:
                if context is not None:
                    if context.sim is sim:
                        continue
            if self.only_household_sims:
                if context is not None:
                    if context.sim.household_id != sim.household_id:
                        continue

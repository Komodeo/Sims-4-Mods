...
# Compiled at: 2021-04-01 15:01:44
        ...
        for sim in self._interaction.required_sims():
            for si in sim.si_state:
                if si is not self._interaction.super_interaction:
                    if not si.preserve_held_props_during_other_si:
...
class OwnsPostureLiability(ReplaceableLiability):
    ...
    def should_transfer(self, continuation):
        if not continuation.involves_carry or self._posture_ref is not None:
...
class CancelAOPLiability(Liability):
    ...
    def release(self):
        ...
        sim = self._sim_ref()
        posture = sim.posture
        if not sim is not None or posture is self._posture:
            if posture.is_vehicle or posture.ownableand self._posture.owning_interactionsand len(self._posture.owning_interactions) == 1 and self._posture.owning_interactions:

    ...
    def get_default_callback(interaction_asm_type):
        ...
        def callback(cls, fields, source, *, factory, overrides, actor_participant_type=ParticipantType.Actor, target_participant_type=ParticipantType.TargetSim, **kwargs):
            ...
            for animation_element_factory in factory.animation_element_gen():
                ...
                if not interactions.interaction_instance_manager.BUILD_AC_CACHE:
                    if cls.resource_key not in sims4.resources.localwork_no_groupid:
                        if asm_key not in sims4.resources.localwork_no_groupid:
                            if caches.USE_ACC_AND_BCC & caches.AccBccUsage.ACC:
                                return
                ...
                try:
                    animation_constraint_actor = create_animation_constraint(asm_key,
                      actor_name, target_name, carry_target_name, create_target_name,
                      initial_state, begin_states, end_states, total_overrides,
                ...
                if target_name is not None:
                    ...
                    if target_actor_definition.actor_type == ASM_ACTORTYPE_SIM:
                        if not target_actor_definition.is_virtual:
                            animation_constraint_target = create_animation_constraint(asm_key,
                              target_name, actor_name, carry_target_name, create_target_name,
                              initial_state, begin_states, end_states, total_overrides,
                              base_object_name=base_object_name)

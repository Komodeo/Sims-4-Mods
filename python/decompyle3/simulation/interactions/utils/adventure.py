...
# Compiled at: 2021-04-28 10:08:36
...
class AdventureMoment(HasTunableFactory, AutoFactoryInit):
    ...
    def _apply_action_cost(self, action):
        if action.cost is not None:
            if action.cost.cost_type == self.COST_TYPE_SIMOLEONS:
                if not self._sim.family_funds.try_remove((action.cost.amount), (Consts_pb2.TELEMETRY_INTERACTION_COST),
                  sim=(self._sim)):
                    return False
            else:
    ...
    def _run_action_from_index(self, action_index):
        ...
        if not (action_result is not None or finish_action.action_results):

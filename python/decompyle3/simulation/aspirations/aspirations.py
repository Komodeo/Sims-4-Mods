...
class AspirationTracker(data_tracker.EventDataTracker, SimInfoTracker):
            ...
            if addtl_obj.owner is not None:
                if owner is not None:
                    if addtl_obj.owner is not owner:
                        ...
                        return
            if not addtl_obj.owner is None or owner is not None:
    ...
    def _should_handle_event(self, milestone, event, resolver):
        ...
        if aspiration.aspiration_type == AspriationType.CAREER:
            ...
            if not (actor is None or actor.is_sim):
                return False
    ...
    def _should_test_objective(self, aspiration, objective):
        objective_data = self._additional_objectives.get(aspiration, None)
        if not objective_data is None:
            if objective not in objective_data.objectives or objective_data.owner is None:
                return True
        ...
        if aspiration_type == AspriationType.FULL_ASPIRATION:
            ...
            if aspiration_type == AspriationType.FAMILIAL:
                ...
                if aspiration_type == AspriationType.WHIM_SET:
                    ...
                    if aspiration_type == AspriationType.NOTIFICATION:

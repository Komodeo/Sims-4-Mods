...
# Compiled at: 2021-06-29 14:20:13
...
class ObjectRouteFromTargetObject(_ObjectRoutingBehaviorBase):
            ...
            for part in target.parts:
                if self.target_selection_test:
                    ...
                    if not self.target_selection_test.run_tests(resolver):
                        continue
                    if self.target_part.subroot_index is not None:
                        if not part.subroot_index is None:
                            if part.subroot_index != self.target_part.subroot_index:
                                continue
                    if part.part_definition is self.target_part.part_definition:
                        if part not in self._unavailable_parts:
    ...
    def _find_target(self):
        ...
        if self.target_participant is not None:
            ...
            objects = [target.get_sim_instance() if target.is_instanced() else target for target in objects if target is not None if target.is_sim]
        ...
        if not connections or allow_caching:
        ...
        if self.target_action_rules.cancel_if_route_incomplete:
            ...
            for target_action_rule in self.target_action_rules.actions:
                ...
                if not target_action_rule.test.run_tests(resolver):
                    continue
                elif target_action_rule.actions is not None:

...
# Compiled at: 2020-05-06 15:26:18
...
target is NoneValueError'No craftable created for {} on {}'.format(self._chosen_recipe, self)            else:
                if self._chosen_object is not None:
                    target = self.owner._create_object_for_situation(sim, self._chosen_object)
target is NoneValueError'No object created for {} on {}'.format(self._chosen_object, self)                else:
...
class WalkbyConsumerSituation(situations.situation_complex.SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, StartingState, factory=(cls._starting_state)),

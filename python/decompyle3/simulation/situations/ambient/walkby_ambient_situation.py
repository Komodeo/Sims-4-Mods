...
class WalkbyAmbientSituation(WalkbyLimitingTagsMixin, SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, _LeaveState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.walker_job.situation_job, cls.walker_job.walkby_state)]
    ...
    def get_sim_available_for_walkby_flavor(self):
        if not (self._cur_state is None or self._cur_state._is_available_for_interruption()):
            return
        ...
        if self._walker is not None:
            if self._walker.opacity < 1.0:
                return
        if self._walker is not None:
            if terrain.is_position_in_street(self._walker.position):
                return
        return self._walker

...
class ServiceNpcSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, ArrivingOnLotSituationState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls._service_npc_job.job, cls._service_npc_job.role_state)]
    ...
    def _on_remove_sim_from_situation(self, sim):
        if sim is self.service_sim() and not self._is_recurring:
            if not self._is_leaving or services.current_zone().service_npc_service is not None:
                if self._service_npc_type.max_work_duration() == 0 and not services.current_zone().is_zone_shutting_down:

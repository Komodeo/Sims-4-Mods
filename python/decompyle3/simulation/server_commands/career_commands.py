...
def select_career(sim_id=None, career_instance_id=None, track_id=None, level=None, company_name_hash=None, reason=CareerOps.JOIN_CAREER, schedule_shift_type=CareerShiftType.ALL_DAY, _connection=None):
    ...
    if reason == CareerOps.JOIN_CAREER:
        ...
        if current_career is not None:
            ...
            if level >= len(career_track.career_levels):
                logger.error('The career track {} does not have a level {}', career_track,
                  level, owner='jmorrow')
                return False
            career = career_type(sim_info)
            career_tracker.add_career(career, show_confirmation_dialog=True,
              schedule_shift_override=schedule_shift_type,
              career_level_override=(career_track.career_levels[level]),
...
def offer_assignments(opt_sim: OptionalSimInfoParam=None, _connection=None):
    ...
    if sim_info is None:
        ...
        return
    for career in sim_info.career_tracker.careers.values():
        if not (career.on_assignment or career.currently_at_work):

...
class BaseRelationshipTrack:
    ...
    def on_add(self):
        self.tracker.suppress_callback_setup_during_load or self._per_instance_data.setup_callbacks()
        old_bit, new_bit = self.update_instance_data()
        sim_id_a = (self.tracker.load_in_progress or self.tracker.rel_data).sim_id_a
...
class RelationshipTrack(BaseRelationshipTrack, TunedContinuousStatistic, HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
 ...
 L. 875        66  SETUP_LOOP          110  'to 110'
               ...
               74  GET_ITER         
             76_0  COME_FROM           106  '106'
             76_1  COME_FROM            92  '92'
...
Parse error at or near `None' instruction at offset -1
    ...
    def _decay_alarm_callback(self, handle):
        ...
        sim_info_a = self.tracker.rel_data.relationship.find_sim_info_a()
        sim_info_b = self.tracker.rel_data.relationship.find_sim_info_b()
        if not sim_info_a is not None or sim_info_b is not None:

...
def push_object_pick_up_and_consume(parent_crafting_interaction, order_sim, object_to_serve, consume_affordance_override=None):
    if not order_sim is None:
        if order_sim.si_state is None or order_sim.is_being_destroyed:
        ...
        if parent_crafting_interaction.sim is order_sim:
            if not (parent_crafting_interaction.process.orders or parent_crafting_interaction.should_push_consume(check_phase=False)):
                ...
                if runtime_slot.empty:
                    ...
                    for child in child_objects:
                        if child.objectage_component:
                            if not child.in_use_by(self.sim):
...
class ChooseDeliverySuperInteraction(CraftingPhaseSuperInteractionMixin, SuperInteraction):
                        ...
                        if deliver_to_sit_slot is not None:
                            ...
                            if order_surface_target.part_owner is serve_object:
                                if order_surface_target.is_valid_for_placement(definition=(object_info.definition)):
                                    ...
                                    return (
                                     deliver_to_sit_slot, target, deliver_part, carry_track)
    ...
    def _push_consume_affordance(self, order_sim, recipe):
        self._went_to_next_phase_or_finished_crafting = True
        if not (self.process.orders or self.should_push_consume(check_phase=False)):

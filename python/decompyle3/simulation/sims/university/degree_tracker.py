...
class DegreeTracker(SimInfoTracker):
    ...
    def _populate_gpa_thresholds(*args):
        ...
        sorted_grades.sort(key=(lambda x: x[0]
), reverse=True)
    ...
    def _set_enrollment_status(self, enrollment_status, from_complete_term=False):
        ...
        if enrollment_status == EnrollmentStatus.SUSPENDED:
            ...
            if enrollment_status == EnrollmentStatus.NONE or enrollment_status == EnrollmentStatus.DROPOUT or enrollment_status == EnrollmentStatus.NOT_ENROLLED:
    ...
    def get_not_yet_accepted_degrees(self):
        ...
        for university in University.ALL_UNIVERSITIES:
            ...
            if not self._accepted_degrees is None or university_id not in self._accepted_degrees:
    ...
    def is_accepted_degree(self, university, degree):
        if not self._accepted_degrees is None:
            if university.guid64 not in self._accepted_degrees or degree.guid64 not in self._accepted_degrees[university.guid64]:
                return False
            return True

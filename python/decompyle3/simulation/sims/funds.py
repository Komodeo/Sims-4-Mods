...
# Compiled at: 2021-02-25 16:10:20
...
class _Funds:
    ...
    def try_remove_amount(self, amount, reason, sim=None, require_full_amount=True):
        ...
        if not self.allow_npcs:
            if sim is not None:
                if sim.is_npc:
                    return amount
        if require_full_amount == False:
            if amount > self._funds and not self.allow_negative_funds:
                amount = self._funds
            else:
    ...
    def _update_money(self, amount, reason, sim=None, tags=None, count_as_earnings=True, show_fx=True):
        ...
        if self._funds < 0:
            ...
            if not count_as_earnings or amount > 0:
                if sim is None:

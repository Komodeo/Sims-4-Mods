...
class PostureScoring:
    ...
    def get_goal_node_cost(goal_node, sim, interaction, var_map, preferences, included_sis, additional_template_dict, relationship_bonuses, spec_constraint, group_constraint, spec_slots_and_costs):
        ...
        if reservation_handler is not None:
            if goal_body_target is not None and not goal_body_target.is_sim:
                may_reserve_posture_target(sim, goal_body_posture, goal_body_target, interaction=interaction) or cost += PostureScoring.IN_USE_PENALTY
                if gsi_handlers.posture_graph_handlers.archiver.enabled:
                    cost_str_list.append('IN_USE_PENALTY: {}'.format(PostureScoring.IN_USE_PENALTY))
            else:
                if goal_surface_target is not None:
                    if postures.posture_specs.PostureSpecVariable.SLOT in var_map:
                        ...
                        for runtime_slot in runtime_slots:
                            if runtime_slot is not None:
                                ...
                                if best_slot_type_and_cost is not None:
                                    if best_slot_type_and_cost[1] <= slot_cost:
                                        continue
        ...
        if additional_template_dict:
            ...
            if goal_body_target is not None:
                if goal_body_target.is_part:
                    ...
                    if body_target_bonus != 0:
                        if goal_body_target is sim.posture_target:
            ...
            if preferences.apply_posture_costs:
                ...
                if not preferences.prefer_surface or goal_surface_target is not None:

                ...
                for stat in skills_to_check:
                    if skill_tag in stat.tags:
                        ...
                        if threshold.compare(curr_value):
                            num_passed += 1
                        if curr_value > highest_skill_value:
...
class SkillThreshold(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def skill_range_max(self):
        ...
        if not comparison_operator == sims4.math.Operator.LESS_OR_EQUAL:
            if comparison_operator == sims4.math.Operator.LESS or comparison_operator == sims4.math.Operator.EQUAL:
                return self.skill_threshold.value
    ...
    def skill_range_min(self):
        comparison_operator = sims4.math.Operator.from_function(self.skill_threshold.comparison)
        if not comparison_operator == sims4.math.Operator.GREATER_OR_EQUAL:
            if comparison_operator == sims4.math.Operator.GREATER or comparison_operator == sims4.math.Operator.EQUAL:
                return self.skill_threshold.value
            return 0

...
class FestivalContestDramaNodeMixin:
    ...
    def add_score(self, sim_id, object_id, score):
        ...
        if sim_id == 0:
            ...
            if not found_score:
                scores.append(ContestScore(score, sim_id=sim_id, object_id=object_id))
        scores.sort(key=(lambda item: item.score * self._score_multipliers.get(item.sim_id, 1.0)
), reverse=True)
    ...
    def get_scores_gen(self):
        self._scores.sort(key=(lambda item: item.score * self._score_multipliers.get(item.sim_id, 1.0)
), reverse=False)
    ...
    def award_winners(self, show_fallback_dialog=False):
        ...
        while 1:
            for node in possible_nodes:
                if hasattr(node, '_running_contest_sub_nodes'):
                    if rootnode.uid in node._running_contest_sub_nodes:
                        rootnode = node
                        break
            else:
                break

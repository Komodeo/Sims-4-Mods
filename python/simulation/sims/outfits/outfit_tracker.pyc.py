...
class OutfitTrackerMixin:
    ...
    def add_outfit(self, outfit_category: OutfitCategory, outfit_data):
        ...
        return (
         OutfitCategory(outfit_category), outfit_index)
    ...
    def _get_random_daily_outfit(self, outfit_category):
        ...
        if existing_default:
            if current_time.absolute_days() - last_randomize_time.absolute_days() >= 1 or current_time.day() != last_randomize_time.day():
                index = 0
    ...
    def handle_career_outfits(self):
        ...
        for career in sim.careers.values():
            if career.has_outfit():
                if not sim.has_outfit((OutfitCategory.CAREER, 0)):
 ...
 outfit_category_and_index, interaction=interaction, change_reason=change_reason, **kwargs), flush_all_animations)
        ...
        if reason in OutfitTuning.OUTFIT_CHANGE_REASONS:
            ...
            for test_group_and_outfit in test_group_and_outfit_list:
                ...
                if outfit_category == OutfitCategory.BATHING:
                    ...
                    if not (outfit_category != OutfitCategory.CURRENT_OUTFITand interaction is not None and interaction is not None and interaction.career_uid):
    ...
    def _run_career_fixup(self, outfit_change, interaction):
        ...
        if not self._career_tracker.has_part_time_career_outfit():
            if not self._career_tracker.has_school_outfit():
                return outfit_change
        if not (outfit_change[0] != OutfitCategory.CAREER or interaction is None):
            if not hasattr(interaction, 'career_uid') or interaction.career_uid == None:
                return outfit_change
            if interaction.career_uid is not None:

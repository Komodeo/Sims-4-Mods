...
# Compiled at: 2020-11-12 07:43:21
    ...
    class _SpecificMajors(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def is_valid_major(self, sim_info, tooltip=None):
            ...
            if not self._majors.test_item(major):
                return TestResult(False, "{0}'s enrolled majors do not match the tuned whitelist/blacklist.", sim_info,
                  tooltip=tooltip)
    ...
    def is_valid(self, sim_info, tooltip=None):
        if self.major is not None:
            ...
            if self.enrollment_status is not None:
                if not self.enrollment_status.test_item(degree_tracker._enrollment_status):
                    return TestResult(False, '{0} does not pass the whitelist/blacklist for university enrollment status', sim_info,
                      tooltip=tooltip)
            return TestResult.TRUE
    ...
    class AcceptedPrestigeDegreeCountTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if not self.number_to_test.compare(num_acc_prestige_degrees):
                return TestResult(False, '{} of accepted prestige degrees, failed threshold {}:{}.', num_acc_prestige_degrees,

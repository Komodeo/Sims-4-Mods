...
# Compiled at: 2020-06-10 09:40:35
...
class UniversityUtils:
    ...
    def _get_common_kick_out_reason(sim_info):
        ...
        if not sim_info is None:
            if sim_info.degree_tracker is None or sim_info.degree_tracker.is_immune_to_kickout:
    ...
    def validate_household_roommate(sim_id, zone_id):
        ...
        if not services.sim_filter_service().does_sim_match_filter(sim_id, additional_filter_terms=filter_terms,
          gsi_source_fn=(lambda: 'UniversityUtils: Valid roommate based on {}'.format(str(zone_id))
)):
            return False
        return True
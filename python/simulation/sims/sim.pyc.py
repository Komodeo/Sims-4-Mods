...
class Sim(HasSimInfoMixin, LockoutMixin, EnvironmentScoreMixin, GameObject):
            ...
            for transition_spec in remaining_transitions:
                if transition_spec.portal_obj is not None:
                    ...
                    if outfit_category_and_index is None:
                        continue
                    elif outfit_category_and_index[0] in OutfitTuning.INAPPROPRIATE_STREETWEAR:
    ...
    def get_location_for_save(self):
        ...
        if self.location.routing_surface is not None:
            ...
            surface_id = 1
        return (
         position, orientation, level, surface_id)
    ...
    def notify_social_group_changed(self, group):
        if self in group:
            ...
            if group in self._social_groups:
    ...
    def on_state_changed(self, state, old_value, new_value, from_init):
        ...
        if old_value != new_value:
            if state == ConsumableComponent.FAT_STATE or state == ConsumableComponent.FIT_STATE:
                ...
                if state == ConsumableComponent.FAT_STATE:
                    ...
                    if new_fat > midrange_fat:
                        ...
                        if new_fat == fat_commodity.min_value:
        ...
        if self.transition_controller is not None:
            if self.routing_component.current_path is not None and self.routing_component.current_path.nodes.needs_replan():
                if self.transition_controller.succeeded:
                    ...
                    if self.routing_component.current_path.final_location.transform != self.routing_component.current_path.intended_location.transform and not self.validate_location(self.routing_component.current_path.intended_location):
    ...
    def validate_location(self, location, from_reset=False):
        ...
        if routing_location.routing_surface.type == routing.SurfaceType.SURFACETYPE_POOL:
            ...
            if wading_interval is None and not water_depth > 0:
                if not wading_interval is not None or water_depth >= wading_interval.upper_bound:
                    return False
        allowed_targets = set()
        ...
        if not from_reset:
            if on_object_surface and not self.posture.unconstrained:
    ...
    def validate_current_location_or_fgl(self, from_reset=False):
        parent = self.parent
        if parent is not None:
            if not (parent.is_sim or from_reset):
                return
    ...
    def get_location_on_nearest_surface_below(self):
        if self.posture_state.valid:
            if self.posture.unconstrained and not self.posture.consider_constrained_for_on_surface_checks:
                if not (self.active_transition is not None and self.active_transition.source.unconstrained):
                    return (
                     self.location, True)
        ...
        try:
            try:
                if not being_destroyed:
                    if not self.is_npc or self.sim_info.get_current_outfit()[0] == OutfitCategory.BATHING:
                        if not self.in_pool:
                ...
                self.last_affordance = None
                self.last_animation_factory = None
                if not (being_destroyed or self._is_removed):
                    try:
    ...
    def is_running_interaction(self, affordance, target):
        ...
        return (
         affordance, target) in affordance_targets
                ...
                if join_sim is self:
                    ...
                    if join_sim is context.sim:
                        ...
                        if join_target is None:
                            if interaction.sim is not self:
                                continue
    ...
    def locked_from_obj_by_privacy(self, obj):
        for privacy in services.privacy_service().privacy_instances:
            ...
            if self in privacy.allowed_sims:
                continue
            if self not in privacy.disallowed_sims:
                if privacy.evaluate_sim(self):
                    continue
    ...
    def create_default_si(self, target_override=None):
        ...
        if build_buy.is_location_pool(self.position, self.location.level) or self.routing_surface.type == SurfaceType.SURFACETYPE_POOL:
            ...
            if self.posture.mobile and self.posture.posture_type is not posture_graph.SIM_DEFAULT_POSTURE_TYPE:
    ...
    def scored_stats_gen(self):
        for stat in self.statistic_tracker.all_statistics():
            if stat.is_scored:
                if self.is_scorable(stat.stat_type):
                    yield stat

        for commodity in self.commodity_tracker.all_statistics():
            if commodity.is_scored:
                if self.is_scorable(commodity.stat_type):
                    yield commodity
    ...
    def ignore_group_socials(self, excluded_group=None):
        ...
        if not next_interaction is not None and next_interaction.is_super or next_interaction.ignore_group_socials:
 ...
 L.3673        54  LOAD_FAST                'base_constraint'
               ...
               62  POP_JUMP_IF_FALSE_LOOP    50  'to 50'

 L.3674        64  BREAK_LOOP       
               66  JUMP_LOOP            50  'to 50'
               68  POP_BLOCK        
 ...
 L.3684       150  SETUP_LOOP          276  'to 276'
              ...
              156  GET_ITER         
            158_0  COME_FROM           272  '272'
            158_1  COME_FROM           260  '260'
            158_2  COME_FROM           210  '210'
 ...
 L.3690       200  LOAD_GLOBAL              len
              ...
              206  LOAD_CONST               1
              208  COMPARE_OP               >
              210  POP_JUMP_IF_FALSE_LOOP   158  'to 158'
...
Parse error at or near `CONTINUE' instruction at offset 260
    ...
    def get_next_work(self):
        ...
        if not self.is_simulating:
            ...
            return WorkRequest(work_element=(elements.GeneratorElement(self._startup_sim_gen)), required_sims=(
             self,))
        if not (self.has_work_locks or services.posture_graph_service().has_built_for_zone_spin_up):
            return WorkRequest()
    ...
    def get_idle_element(self, duration=10):
        ...
        return (
         elements.GeneratorElement(do_idle_behavior), cancel_idle_behavior)
    ...
    def set_autonomy_preference(self, preference, obj, context):
        if preference.is_scoring:
            ...
            if preference.use_only:
                ...
                target_objects = set()
                if context is not None:
                    if context.pick is not None and not obj.is_part:
    ...
    def apply_outfit_buffs_for_sim_info(self, sim_info, category_and_index):
        ...
        if not (outfit_data is None or outfit_data.part_ids):
            return
    ...
    def is_on_active_lot(self, tolerance=0, include_spawn_point=False):
        ...
        if not lot.is_position_on_lot(position, tolerance):
            if include_spawn_point and not self._is_on_spawn_point():
                return False
            ...
            if intended_position != position:
                if not lot.is_position_on_lot(intended_position, tolerance):
                    if include_spawn_point and not self._is_on_spawn_point(use_intended_position=True):
                        return False
            return True
        ...
        for si in self.si_state:
            ...
            for potential_target in potential_targets:
                if target is potential_target:
                    break
                if potential_target.is_part:
                    if potential_target.part_owner is target:
                        break
            else:
                continue
    ...
    def try_remove_teleport_style_interaction_to_inject(self, interaction):
        ...
        if current_ref_count <= 0:
            ...
            self._teleport_style_interactions_to_inject = self._teleport_style_interactions_to_inject or None
        else:
    ...
    def get_teleport_style_interaction_aop(self, interaction, override_pick=None, override_target=None):
        ...
        for teleport_style_affordance in teleport_style_affordances:
            ...
            if test_result:
                ...
                return (
                 aop, interaction_context, teleport_style_data)

...
class AnimationOverrides:
    ...
    def __bool__(self):
        if not self.params:
            if self.vfx or self.sounds or self.props or self.prop_state_values or self.manifests or self.required_slots or self.balloons or self.reactionlet or self.animation_context:
                return True
            return False
...
def partition_boundary_on_params(boundary_to_params):
    ...
    def get_matching_params_excluding_key(k, v):
        ...
        for boundary, param_sets in boundary_to_params.items():
            ...
            for params in param_sets:
                vp = params.get(k, v)
                if vp == v:
                    valid_params.add(sims4.collections.frozendict({kf: vf for kf, vf in  if kf != k}))
    ...
    for k, vs in ks_to_vs.items():
        ...
        for v in vs:
            ...
            if matching_params != matching_params_v:
                unique_keys.add(k)
                break

    boundary_param_sets = {boundary: unique_keys for boundary in }
    ...
    if target is not None:
        if override_list is not None:
            ...
            for tested_animation in override_list:
                if tested_animation.override is not None:
                    if tested_animation.tests is not None:
                        if tested_animation.tests.run_tests(resolver):

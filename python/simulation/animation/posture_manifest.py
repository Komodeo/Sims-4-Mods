...
class PostureManifestEntry(_PostureManifestEntry, InternMixin):
        ...
        if specific != MATCH_ANY:
            if specific != MATCH_NONE or self.specific is not None:
                if self.specific != specific:
                    return False
            if not family != MATCH_ANY or family != MATCH_NONE or self.family is not None:
    ...
    def get_entries_with_override(self, override_value: PostureManifestOverrideValue):
        ...
        if not left is None:
            if not self.left == MATCH_ANY or left != MATCH_NONE:
                left = self.left
    ...
    def _intersect_attr(value0, value1):
        ...
        if value0 == value1:
            return value0
        if not (isinstance(value0, (str, int, Definition)) or isinstance(value1, (str, int, Definition))):
            if value0.is_part:
                if value1.parts:
            ...
            for i, self_entry in enumerate(self_entries):
                ...
                if self_entry.actor == other_entry.actor:
                    if not self_entry.slot_types.intersection(other_entry.slot_types):
                        ...
                        needs_add = False
                    continue
                if self_entry.slot_types.intersection(other_entry.slot_types):
                    return

...
# Compiled at: 2020-10-23 15:18:19
...
def is_prescribed_weather_supported(prescribed_weather_type, options, thunder_snow_storms_enabled, snow_safe=True, rain_safe=True):
    ...
    if prescribed_weather_type.rain:
        if not rain_safe or options[PrecipitationType.RAIN] == WeatherOption.WEATHER_DISABLED:
            return False
    if prescribed_weather_type.snow:
        if not snow_safe or options[PrecipitationType.SNOW] == WeatherOption.WEATHER_DISABLED:
            return False
    if prescribed_weather_type.storm:
        if not rain_safe or options[PrecipitationType.RAIN] == WeatherOption.DISABLE_STORMS:
            return False
    if prescribed_weather_type.blizzard:
        if not snow_safe or options[PrecipitationType.SNOW] == WeatherOption.DISABLE_STORMS:
            return False
    if prescribed_weather_type.thunder_snow_storms:
        if snow_safe and not thunder_snow_storms_enabled:
            return False
        return True
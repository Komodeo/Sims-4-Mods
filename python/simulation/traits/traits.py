...
class Trait(HasTunableReference, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, HasTunableLodMixin, MixerActorMixin, MixerProviderMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.TRAIT)):
    ...
    def _verify_tuning_callback(cls):
        ...
        if cls.day_night_tracking is not None:
            if not cls.day_night_tracking.sunlight_buffs:
                cls.day_night_tracking.shade_buffs or cls.day_night_tracking.day_buffs or cls.day_night_tracking.night_buffs or logger.error('Trait {} has Day Night Tracking enabled but no buffs are tuned. Either tune buffs or disable the tracking.', cls, owner='BadTuning')
            else:
                pass
            tracking_buff_tag = Trait.DAY_NIGHT_TRACKING_BUFF_TAG
            if any((buff for buff in cls.day_night_tracking.sunlight_buffs if not buff.buff_type.has_tag(tracking_buff_tag) if  if not buff.buff_type.has_tag(tracking_buff_tag))) or any((buff for buff in cls.day_night_tracking.shade_buffs if not buff.buff_type.has_tag(tracking_buff_tag) if  if not buff.buff_type.has_tag(tracking_buff_tag))) or any((buff for buff in cls.day_night_tracking.day_buffs if not buff.buff_type.has_tag(tracking_buff_tag) if  if not buff.buff_type.has_tag(tracking_buff_tag))) or any((buff for buff in cls.day_night_tracking.night_buffs if not buff.buff_type.has_tag(tracking_buff_tag) if  if not buff.buff_type.has_tag(tracking_buff_tag))):
            ...
            for commodity_info in cls.conditional_commodities:
                if not commodity_info.commodity.persisted_tuning:
                    logger.error('Trait {} has a conditional initial commodity {} that does not have persisted tuning.', cls, commodity_info.commodity)
                if len(commodity_info.tests) == 0:
                    logger.error('Trait {} has a conditional initial commodity {} that does not have any tests.', cls, commodity_info.commodity)

        if cls.is_preference_trait:
            if cls.trait_type not in PreferenceTypes:
                logger.error('Trait {} is a preference but has invalid trait type {}', cls, cls.trait_type)
        else:
            if cls.trait_type in PreferenceTypes:
                logger.error('Trait {} is NOT a preference but has a preference trait type {}', cls, cls.trait_type)
    ...
    def is_valid_trait(cls, sim_info_data):
        ...
        if cls.genders:
            if sim_info_data.gender not in cls.genders:
                return False
        if cls.species:
            if sim_info_data.species not in cls.species:
                return False
        return True
    ...
    def is_conflicting(cls, trait):
        ...
        if cls.conflicting_traits:
            if trait in cls.conflicting_traits:
                return True
        if trait.conflicting_traits:
            if cls in trait.conflicting_traits:
                return True
        return False

...
# Compiled at: 2020-11-17 19:57:36
...
class RouteEvent(RouteEventBase, HasTunableReference, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(resources.Types.SNIPPET)):
    ...
    def test(cls, resolver, from_update=False):
        ...
        if not result:
            return result
        actor = (from_update or resolver.get_participant)(ParticipantType.Actor)
        if actor is not None:
            actor = actor.sim_info.get_sim_instance() if actor.is_sim else actor

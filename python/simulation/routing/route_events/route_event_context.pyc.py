    ...
    def fill_and_get_start_time_for_route_event(self, route_event, path, repeat_event=False, schedule_preference=RouteEventSchedulePreference.BEGINNING):
        ...
        for idx, bucket in enumerate(buckets):
            ...
            if bucket.end_time - duration < earliest_time:
                continue
            if time is not None:
                if not bucket.start_time > time:
                    if bucket.end_time - duration < time:
                        continue
            if straight_path_tuning is None:
                if time is not None:
                    break
                elif schedule_preference == RouteEventSchedulePreference.BEGINNING:
...
class RouteEventContext:
    ...
    def prune_stale_events_and_get_failed_types(self, actor, path, current_time):
        ...
        for route_event, time in self._scheduled_events:
            if not (type(route_event) in failed_event_types or route_event.test((route_event.get_resolver(actor)), from_update=True)):
                failed_events.append((route_event, time))
                failed_event_types.add(type(route_event))
            if not route_event.is_route_event_valid(route_event, time, actor, path):
                failed_events.append((route_event, time))
    ...
    def _test_gathered_events_for_chance(self, actor):
        ...
        for route_event_type, route_events in self._route_events_to_schedule.items():
            for route_event in tuple(route_events):
                ...
                if route_event_type != RouteEventType.LOW_REPEAT:
                    if random() > route_event.chance.get_chance(resolver):
    ...
    def schedule_route_events(self, actor, path, failed_event_types=None, start_time=0):
        ...
        for route_event_type in reversed(RouteEventType):
            if route_event_type == RouteEventType.FIRST_INDOOR:
                ...
                if route_event_type == RouteEventType.LAST_INDOOR:
                    ...
                    if route_event_type == RouteEventType.FIRST_OUTDOOR:
                        ...
                        if route_event_type == RouteEventType.LAST_OUTDOOR:
                            _schedule_route_events(route_event_type)
                        else:
                            if route_event_type == RouteEventType.INTERACTION_PRE:
                                _schedule_route_events(route_event_type, schedule_preference=(RouteEventSchedulePreference.END))
                            else:
                                if route_event_type == RouteEventType.INTERACTION_POST:
                                    _schedule_route_events(route_event_type)
                                else:
                                    if route_event_type == RouteEventType.ENTER_LOT_LEVEL_INDOOR:
                                        _schedule_route_events(route_event_type)
                                    else:
                                        if route_event_type == RouteEventType.BROADCASTER:
                                            _schedule_route_events(route_event_type)
                                        else:

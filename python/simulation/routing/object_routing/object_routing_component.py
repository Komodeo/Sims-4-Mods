...
# Compiled at: 2021-06-29 11:37:37
...
class ObjectRoutingComponent(FootprintToggleMixin, Component, HasTunableFactory, AutoFactoryInit, component_name=types.OBJECT_ROUTING_COMPONENT):
    ...
    def _remove_target_callbacks(self):
        ...
        object_manager = services.object_manager()
        if object_manager is not None:
            if object_manager.is_callback_registered(CallbackTypes.ON_OBJECT_REMOVE, self._on_target_removed):
    ...
    def violates_privacy(self, privacy):
        if not self.privacy_rules:
            return False
        if not privacy.vehicle_violates_privacy(self.owner):
            return False
        return True
    ...
    def _set_running_behavior(self, new_behavior):
        ...
        if self.tracking_category:
            if self.tracking_category is not ObjectRoutingBehaviorTrackingCategory.NONE:
    ...
    def get_idle_element(self):
        ...
        return (
         self._idle_element, self._cancel_idle_behavior)

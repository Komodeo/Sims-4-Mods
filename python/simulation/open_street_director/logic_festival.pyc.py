...
# Compiled at: 2016-09-26 14:28:44
...
class LogicFestivalContestSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, LogicFestivalContestState, factory=(cls.contest_state)),)

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.npc_job_and_role_state.job, cls.npc_job_and_role_state.role_state),
         (
...
class ScoredLogicFestivalContestSituation(LogicFestivalContestSituation):
    ...
    def _get_end_notification_resolver_and_tokens(self):
        ...
        sims.sort(key=(lambda item: (item.get_stat_value(self.contest_statistic), item.is_selectable)
), reverse=True)
        gold_sim = sims[0]
        return (
         SingleSimResolver(gold_sim.sim_info), (gold_sim.get_stat_value(self.contest_statistic),))
...
class LogicFestivalOpenStreetDirector(BaseFestivalOpenStreetDirector):
    ...
    def _states(cls):
        return (
         FestivalStateInfo(SpinupFestivalState, cls.spinup_festival_state),

...
class AutonomyMode:
    ...
    def _score_motives(self):
        motive_scores = None
        motive_scores = {stat.stat_type: ScoredStatistic(stat, self._sim) for stat in }
            ...
            if not self._is_available(target_sim):
                ...
                for affordance in target_sim.super_affordances():
                    ...
                    if not test_result:
                        ...
                        if not target_sim_si_state.is_compatible(interaction, (interaction.priority), group_id=(interaction.group_id),
        ...
        if not self._actively_scored_motives:
            ...
            while True:
                ...
                if not motives_to_score:
                    ...
                    self._actively_scored_motives = {stat.stat_type for stat in itertools.takewhile(lambda desire: self._motive_scores[desire] >= variance_score
, motives_to_score)}
                if not self._actively_scored_motives:
                    break
    ...
    def _should_score_object_inventory(self, inventory_component):
        ...
        if InventoryTypeTuning.is_shared_between_objects(inventory_type):
            if inventory_type in self._scored_shared_inventory_types:
                return False
        return True
    ...
    def _score_object_interactions_gen(self, timeline, obj, timeslice_if_needed_gen, inventory_type, best_threshold):
        ...
        for aop in potential_interactions:
            ...
            if not test_result:
                if not self._GSI_IGNORES_NON_AUTONOMOUS_AFFORDANCES:
                    if self._request.record_test_result is not None:
        ...
        if autonomy_preference is not None:
            if not autonomy_preference.preference.is_scoring:
                ...
                if preference_type == AutonomyPreferenceType.DISALLOWED:
                    if self._request.context.source != InteractionContext.SOURCE_PIE_MENU:
                        return (
                         InteractionResult.Failure(aop, (AutonomyStageLabel.BEFORE_POSTURE_SEARCH), (self._actively_scored_motives), reason='Disallowed by autonomy preference'), None, 0)
            ...
            if posture_result_tuple is None:
                ...
                try:
                    ...
                    try:
                        ...
                        return (
                         InteractionResult.Failure(aop, (AutonomyStageLabel.BEFORE_POSTURE_SEARCH), (self._actively_scored_motives), reason='Posture Graph Threw Exception'), None, 0)
                    finally:
                        exception = None
                        del exception
                ...
                if situation_type_social_score_multiplier is not None:
                    ...
                    for situation_type, multipliers in situation_type_social_score_multiplier.items():
                        for situation in situations_sim_is_in:
                            if type(situation) == situation_type:
                                if target_sim in situation.all_sims_in_situation_gen():
        ...
        try:
            if aop.target is None or aop.target.parts is None:
                if interaction.target is None or interaction.target.parts is None:
                    ...
                    context = self._request.context
            if context.source == context.SOURCE_AUTONOMY:
                if not self._request.consider_scores_of_zero:
    ...
    def _applicable_stat_ops_gen(self, interaction, include_hidden_false_ads=False, gsi_commodity_scores=None):
        for stat_op_list in interaction.affordance.autonomy_ads_gen(target=(interaction.target), include_hidden_false_ads=include_hidden_false_ads):
            if not stat_op_list.stat.add_if_not_in_tracker or stat_op_list.stat not in self._motive_scores:
                if gsi_commodity_scores is not None:
    ...
    def _get_motives_to_score(self):
        ...
        if not (self._request.has_commodities or self._request.affordance_list):
            ...
            motives_to_score = frozenset((stat.stat_type for stat in itertools.takewhile(lambda desire: self._motive_scores[desire] >= variance_score
, stats)))
    ...
    def _calculate_object_stat_use_multiplier--- This code section failed: ---

 L.2679         0  LOAD_FAST                'game_object'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_FALSE    12  'to 12'

 L.2680         8  LOAD_CONST               1
               10  RETURN_VALUE     
             12_0  COME_FROM             6  '6'

 L.2682        12  LOAD_CONST               1
               14  STORE_FAST               'final_multiplier'

 L.2683        16  SETUP_LOOP          114  'to 114'
               18  LOAD_FAST                'game_object'
               20  LOAD_ATTR                autonomy_modifiers
               22  GET_ITER         
             24_0  COME_FROM           110  '110'
             24_1  COME_FROM            98  '98'
             24_2  COME_FROM            86  '86'
             24_3  COME_FROM            54  '54'
             24_4  COME_FROM            34  '34'
               24  FOR_ITER            112  'to 112'
               26  STORE_FAST               'autonomy_modifier'

 L.2685        28  LOAD_FAST                'autonomy_modifier'
               30  LOAD_ATTR                statistic_multipliers
               32  POP_JUMP_IF_TRUE     36  'to 36'

 L.2686        34  CONTINUE             24  'to 24'
             36_0  COME_FROM            32  '32'

 L.2689        36  LOAD_FAST                'autonomy_modifier'
               38  LOAD_ATTR                statistic_multipliers
               40  LOAD_METHOD              get
               42  LOAD_FAST                'stat_type'
               44  CALL_METHOD_1         1  '1 positional argument'
               46  STORE_FAST               'stat_use_multiplier'

 L.2690        48  LOAD_FAST                'stat_use_multiplier'
               50  LOAD_CONST               None
               52  COMPARE_OP               is-not
               54  POP_JUMP_IF_FALSE_LOOP    24  'to 24'

 L.2691        56  LOAD_FAST                'stat_use_multiplier'
               58  LOAD_ATTR                apply_direction
               60  LOAD_GLOBAL              StatisticChangeDirection
               62  LOAD_ATTR                BOTH
               64  COMPARE_OP               ==
               66  POP_JUMP_IF_TRUE    100  'to 100'

 L.2692        68  LOAD_FAST                'fulfillment_rate_is_increasing'
               70  POP_JUMP_IF_FALSE    84  'to 84'
               72  LOAD_FAST                'stat_use_multiplier'
               74  LOAD_ATTR                apply_direction
               76  LOAD_GLOBAL              StatisticChangeDirection
               78  LOAD_ATTR                INCREASE
               80  COMPARE_OP               ==
               82  POP_JUMP_IF_TRUE    100  'to 100'
             84_0  COME_FROM            70  '70'

 L.2693        84  LOAD_FAST                'fulfillment_rate_is_increasing'
               86  POP_JUMP_IF_TRUE_LOOP    24  'to 24'
               88  LOAD_FAST                'stat_use_multiplier'
               90  LOAD_ATTR                apply_direction
               92  LOAD_GLOBAL              StatisticChangeDirection
               94  LOAD_ATTR                DECREASE
               96  COMPARE_OP               ==
               98  POP_JUMP_IF_FALSE_LOOP    24  'to 24'
            100_0  COME_FROM            82  '82'
            100_1  COME_FROM            66  '66'

 L.2694       100  LOAD_FAST                'final_multiplier'
              102  LOAD_FAST                'stat_use_multiplier'
              104  LOAD_ATTR                multiplier
              106  INPLACE_MULTIPLY 
              108  STORE_FAST               'final_multiplier'
              110  JUMP_LOOP            24  'to 24'
              112  POP_BLOCK        
            114_0  COME_FROM_LOOP       16  '16'

 L.2696       114  LOAD_FAST                'final_multiplier'
              116  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `COME_FROM_LOOP' instruction at offset 114_0
...
class InteractionScore(float):
    ...
    def __str__(self):
        ...
        if self.base_multitasking_percentage is not None:
            final_multitasking_percentage = str(self.base_multitasking_percentage + self.bonus_multitasking_percentage - self.penalty_multitasking_percentage) if (self.bonus_multitasking_percentage is not None) and (self.penalty_multitasking_percentage is not None) else 'None'

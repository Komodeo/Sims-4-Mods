...
# Compiled at: 2019-12-02 16:35:10
    ...
    def objects_to_score_gen(self, motives: set=singletons.DEFAULT):
        ...
        def is_valid_to_score(obj):
            if self.ignored_object_list:
                if obj in self.ignored_object_list:
                    return False
            if not (motives is singletons.DEFAULT and obj.commodity_flags):
                if motives and not obj.commodity_flags & motives:
                    if any((provided_affordance_data.object_filter.is_object_valid(obj) for provided_affordance_data in provided_affordance_datas)) or obj.is_sim and self._check_sim_relbits_commodity_flags(obj, motives):
                        return True
                return False
        ...
        if self.object_list:
            ...
            for obj in services.object_manager().valid_objects():
                if is_valid_to_score(obj):
                    if self._check_object_connectivity_and_rules(obj, autonomy_rule):
                        yield obj
...
class PrerollAutonomyRequest(AutonomyRequest):
    ...
    def objects_to_score_gen(self, motives: set=singletons.DEFAULT):
        if self.object_list:
            for obj in self.object_list:
                if not motives is singletons.DEFAULT:
                    if obj.preroll_commodity_flags & motives:
                        pass
                if self.ignored_object_list:
                    if obj not in self.ignored_object_list:
                        pass
                    yield obj

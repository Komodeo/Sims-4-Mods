...
class CraftingQualityLiability(Liability):
    ...
    def get_quality_state_and_value(self):
        ...
        return (
         quality_state.state_star_number, quality_stats_value)
 ...
 CraftingTuning.STATIC_CRAFTING_COMMODITY,), 
    ...
    def pay_for_item(self):
        if self._paying_sim is not None:
            ...
            for bucks_type, refund_on_cancel in self.recipe.crafting_bucks_refund_amounts.items():
                ...
                if tracker is not None:
                    if tracker.try_modify_bucks(bucks_type, (-amount), reason=(str(self.recipe.final_product_definition_id))):
                        if refund_on_cancel:
        ...
        if should_save_ingredients:
            if self._reserved_ingredients:
                ...
                return
        if self._reserved_ingredients:
            if not (self.recipe.use_ingredients and self.recipe.use_ingredients.return_on_cancel):
    ...
    def phase(self, new_phase):
        ...
        if old_phase != new_phase:
            ...
            if not new_phase is None or old_phase is not None:
    ...
    def pop_order(self):
        ...
        return (
         sim_info.get_sim_instance(), recipe)
    ...
    def setup_crafted_object(self, crafted_object, *, use_base_recipe=False, is_final_product=False, owning_household_id_override=None, random=None):
        ...
        if owning_household_id_override is None:
            ...
            if self.recipe.use_active_household_as_owner or any((role_state.active_household_crafting_override for role_state in crafter_roles)):
                ...
                if any((role_state.lot_owner_crafting_override for role_state in crafter_roles)):
    ...
    def _find_phase_affordances(self, phases, sim, context, default_target, display_errors=True, **kwargs):
        ...
        if affordance_list:
            ...
            if not self.original_target is not None or self.original_target is not sim:
                if self.original_target.is_in_sim_inventory(sim=sim) or self.original_target.is_connected(sim):
    ...
    def _choose_phase_and_create_aop_and_context(self, phases, sim, context, default_target, previous_phase_si, liabilities=None, **kwargs):
        ...
        if phase.target_ico:
            target = self.current_ico if self.current_ico is not None else target_interaction.target
            if gsi_handlers.autonomy_handlers.archiver.enabled:
 ...
 target_affordance, None, crafting_process=self, 
    ...
    def push_si_for_current_phase(self, previous_phase_si, from_resume=False, next_phases=None) -> ExecuteResult:
        ...
        if from_resume:
            liabilities = (
             (
              CRAFTING_QUALITY_LIABILITY, CraftingQualityLiability(self, created_by='Resume')),)
        if next_phases is None:
            if from_resume or not self.should_repeat_phase or self.next_phases:
                phase = None
                if self.orders:
                    _, recipe = self.orders[0]
    ...
    def load(self, crafting_process_message):
        ...
        for phase in self.recipe.phases.values():
            if phase.id == crafting_process_message.phase_id:
                self.phase = phase
            if phase.id == crafting_process_message.previous_phase_id:

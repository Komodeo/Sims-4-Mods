...
class Recipe(metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.RECIPE)):
    ...
    def _tuning_loaded_callback(cls):
        ...
        cls.phases = {phase_id: phase(recipe=cls, phase_id=phase_id) for phase_id, phase in }
        first_phases = []
    ...
    def _verify_recipe_tuning_callback(cls):
        if cls.use_ingredients:
            ...
            if cls.available_in_restaurant:
                if cls.restaurant_menu_icon_definition is None:
                    if cls.icon_override is None:
    ...
    def get_price(cls, is_retail=False, ingredient_modifier=1, multiplier=1):
        ...
        return (
         original_price, int(discounted_price), int(discounted_ingredients_price))

...
class ParticipantRanInteractionTest(event_testing.test_base.BaseTest):
        ...
        for sim_info in sims:
            ...
            if not tag_match:
                if not interaction.affordance in self._all_affordances:
                    if hasattr(interaction.affordance, 'proxied_affordance') and not interaction.affordance.proxied_affordance in self._all_affordances:
            ...
            if self.object_states is not None:
                ...
                if self.interaction_outcome is not None:
                    ...
                    if self.interaction_outcome == OutcomeResult.SUCCESS:
                        ...
                        if self.interaction_outcome != interaction.global_outcome_result:
    ...
    def get_posture_id(self, sims=None, interaction=None):
        if not interaction is None:
            if interaction.sim is None or interaction.sim.posture is None:
                return
    ...
    def target_matches_object_tags(self, interaction=None):
        if not interaction is None:
            if interaction.target is None or interaction.target.is_sim:
                return False
...
class SimoleonsEarnedTest(event_testing.test_base.BaseTest):
    ...
    def __call__(self, sims=None, amount=None, skill_used=None, tags=None):
        ...
        if self.event_type_to_test is not None:
            if not self.event_type_to_test(skill_used, tags):
                    ...
                    for relationship in sim_info.relationship_tracker:
                        for relationship_bit in self.target_family_relationships:
                            if relationship.has_bit(sim_info.sim_id, relationship_bit):
                                ...
                                if target_aspiration_tracker is not None:
                                    if target_aspiration_tracker.milestone_completed(self.aspiration_trigger):
                                        return TestResult.TRUE
...
class GenerationTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self):
        if self.household_threshold is None:
            ...
            household_count = sum((1 for household in services.household_manager().get_all() if not household.hidden if any((self.generation_threshold.compare(sim_info.generation) for sim_info in household)) if  if not household.hidden if any((self.generation_threshold.compare(sim_info.generation) for sim_info in household))))
...
class CareerAttendenceTest(event_testing.test_base.BaseTest):
    ...
    def __call__(self, career=None, data=None, objective_guid64=None):
        ...
        if not self.hours_worked.compare(total_time_worked):
            return TestResultNumeric(False, 'CareerAttendenceTest: not the desired amount of time worked.', current_value=total_time_worked,
...
class TotalRelationshipBitTest(event_testing.test_base.BaseTest):
    ...
    def __call__(self, data_object=None, objective_guid64=None):
        ...
        if not self.num_relations.compare(current_relationships):
            return TestResultNumeric(False, 'TotalRelationshipBitTest: Not enough relationships.',

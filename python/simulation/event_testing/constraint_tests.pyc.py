            ...
            for sim in instanced_sims:
                if total_constraint.geometry.test_transform(sim.transform):
                    if total_constraint.is_routing_surface_valid(sim.routing_surface):
                        if total_constraint.is_location_water_depth_valid(sim.location):
                            if total_constraint.is_location_terrain_tags_valid(sim.location):
                                if self.test_set(DoubleSimResolver(test_actor, sim.sim_info)):
                                    sim_count += 1
                                if self.must_be_line_of_sight:
                                    if not (sim.sim_info is test_actor or object_constraint.intersect(sim.lineofsight_component.constraint).valid):
                                        sim_count += 1
                                    return TestResult(False, 'Sims In Constraint Test Failed.', tooltip=(self.tooltip))

            if self.minimum_sim_count:
                if sim_count < self.minimum_sim_count:

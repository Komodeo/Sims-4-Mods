...
class CollectedItemTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, collection_id=None, collected_item_id=None):
        if self.test_type.test_type == self.COLLECTION_TYPE:
            ...
            if self.test_type.collection_types:
                if collection_id not in self.test_type.collection_types:
    ...
    def __call__(self, subjects=()):
        ...
        if is_work_time:
            if self.is_during_work:
                if self.fail_if_taking_day_off_during_work and not taking_day_off:
                    return TestResult.TRUE
        ...
        if current_gig is not None:
            ...
            if self.after_photos is not None:
                if gig_history is None or self.after_photos == (len(gig_history.after_photos) == 0):
                    return False
                if self.selected_photo_pairs is not None:
                    if gig_history is None or self.selected_photo_pairs == (len(gig_history.selected_photos) == 0):
                        return False
        return True
...
class PostureTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if not value.target is None or value.container_supports is not None or any((posture.multi_sim for posture in value.required_postures)):
            ...
            if not value.target is not None or value.container_supports is None:
        ...
        for household in target_households:
            ...
            if self.additional_bills_delinquency_states is not None:
                for state in self.additional_bills_delinquency_states:
                    ...
                    if household.bills_manager.is_additional_bill_source_delinquent(state.bill_source) != state.is_delinquent:
            ...
            if self.payment_due is not None:
                ...
                if not self.payment_due.is_due and household_payment_due or self.payment_due.utility_consumption_threshold is not None:
        ...
        for participant in participants:
            ...
            if self.count_type == self.COUNT_TESTED_SIM_INFOS:
                ...
                if not self._count.compare(count):
                    return TestResult(False, "Household doesn't meet tested size requirements.",
                      tooltip=(self.tooltip))
                ...
                if subject.careers:
                    ...
                    if not visible_career_found:
                        return TestResult(False, '{0} does not currently have a visible career.', subject,
                          tooltip=tooltip)
                    return story_progression_requirements_met or TestResult(False, '{0} does not currently have a career that supports {1} modifications by the story progression system.', subject,
            ...
            for career in careers_to_check:
                ...
                if hours is None:
                    ...
                    if time_span is None:
                        return TestResultNumeric(False, '{0} does not currently have any hours scheduled in career {1}',
                          subject,
                          career,
                          current_value=(-1),
...
class CareerPromotedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, career=None):
        ...
        if not self.careers.test_item(career):
    ...
    def __call__(self, test_targets=(), additional_reward_parts=None):
        ...
        for target in test_targets:
            if not target.is_sim:
                ...
                household = target.household
            if not all((household.part_in_reward_inventory(reward_part) for reward_part in reward_parts)):
                if self.invert:
                    return TestResult.TRUE
                else:
            ...
            for portal in target.get_portal_instances():
                if sims_to_test is not None:
                    for sim_info in sims_to_test:
                        ...
                        if not sim_flags & required_flags == required_flags:
                            return TestResult(False, "{} sim doesn't have required flags to go through this portal", target, tooltip=(self.tooltip))
...
class EnsembleTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        if self.check_matching_ensemble is None:
            return {'test_targets': self.participant}
        return {'test_targets':self.participant,  'other_participants':self.check_matching_ensemble}

...
class TunableSimTemplate(HasTunableReference, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SIM_TEMPLATE)):
        ...
        for perk in cls._perks.explicit:
            if perk is not None:
                if not cls._perks.whiteblacklist.test_item(perk):
                    logger.error('SimTemplate: {} - explicit perk ({}) failed to meetwhitelist/blacklist requirements.Either update explicit list or whitelist/blacklist', (cls.__name__),
                      (perk.__name__),
                      owner='designer')
            ...
            if sim_info_data['age_range'] is not None:
                ...
                if sim_info_data['gender'] is not None:
                    if sim_info_data['gender'] != sim_creator.gender:
                        return False
                if sim_info_data['species'] is not None:
                    if sim_info_data['species'] != sim_creator.species:
                        return False
    ...
    def _add_skills(cls, sim_info):
        ...
        if cls._skills.random:
            ...
            while num_to_add > 0:
                if available_random_skill_data:
                    if available_skills_types:
    ...
    def _add_traits(cls, sim_info, sim_creator=None):
        ...
        if cls._traits.num_random:
            ...
            if num_to_add > 0:
                ...
                available_trait_types = {trait for trait in  if not sim_info.has_trait(trait)}
    ...
    def add_perks(cls, sim_info, sim_creator=None, suppress_telemetry=False):
        ...
        if cls._perks.num_random:
            ...
            if num_to_add > 0:
                ...
                available_bucks_perk_types = {perk for perk in  if cls._perks.whiteblacklist.test_item(perk)}
                available_bucks_perk_types -= set(cls._perks.explicit)
                available_bucks_perk_types = list(available_bucks_perk_types)
                while num_to_add > 0:
                    if available_bucks_perk_types:

            ...
            for aop in potential_target.potential_interactions(context):
                if tutorial_service is not None:
                    ...
                    if not is_interactable:
                        is_interactable = (aop.affordance.has_pie_menu_sub_interactions)(
...
def _get_targets_from_obj_pick(sim, pick_target, pick_type, position, zone_id, level, lot_id, routing_surface, preferred_objects=()):
    ...
    if lot_id and lot_id != services.active_lot_id():
        ...
        if pick_target is not None:
            if pick_target.provided_routing_surface is not None and not pick_target.is_routing_surface_overlapped_at_position(position):
                potential_targets.append((pick_target, routing_surface))
...
def _get_water_targets_from_pick(sim, position, routing_surface, zone_id, level, is_routable, pool_block_id, pick_target, pick_type):
    ...
    if pool_block_id:
        ...
        if pond_id:
            ...
            if not water_height > 0 or sim is not None and services.terrain_service.ocean_object() is not None:
                if not is_routable:
                    return (None, None, ())
...
def _get_targets_from_pick(sim, pick_target, pick_type, position, level, zone_id, lot_id, is_routable, preferred_objects=()):
    ...
    return (
     pick_target, pick_type, tuple(potential_targets))
...
def generate_choices(target_id=None, pick_type=PickType.PICK_TERRAIN, x=0.0, y=0.0, z=0.0, lot_id=0, level=0, control=0, alt=0, shift=0, reference_id=0, referred_object_id=0, preferred_object_id=0, is_routable=True, _connection=None):
    ...
    if show_pie_menu or show_debug_pie_menu:
        if pick_type == PickType.PICK_PORTRAIT or pick_type == PickType.PICK_CLUB_PANEL:
            ...
            if pick_type == PickType.PICK_SKEWER:
                sim_info = services.sim_info_manager().get(target_id)
                skewer_sim = None
                if sim_info is None:
                    return 0
...
def handle_pie_menu_item_coloring(item_msg, item, sim, choice, name_override_result):
    ...
    if away_action is not None:
        ...
        if item.result.influence_by_active_mood or name_override_result.influence_by_active_mood:

...
def garbage_collector_dump--- This code section failed: ---
 ...
 L. 241   128_130  SETUP_LOOP          458  'to 458'
              ...
              142  GET_ITER         
            144_0  COME_FROM           454  '454'
            144_1  COME_FROM           450  '450'
            144_2  COME_FROM           402  '402'
 ...
 L. 274       436  POP_BLOCK        
            ...
            440_0  COME_FROM_FINALLY   420  '420'
              ...
              446  END_FINALLY      
              448  POP_EXCEPT       
              450  JUMP_LOOP           144  'to 144'
            452_0  COME_FROM           410  '410'
              452  END_FINALLY      
              454  JUMP_LOOP           144  'to 144'
              456  POP_BLOCK        
            ...
            if id(child) not in all_objects:
                ...
                if verbose:
                    if not len(all_objects) % 1000000:
                        output('...{} pending: {}'.formatlen(all_objects)len(pending))
...
def tracemalloc_dump_stats_imp(snapshot, dump_traceback_stats: bool, dump_per_line_stats: bool, _connection=None):
    ...
    if dump_per_line_stats:
        ...
        create_csv('tracemalloc_lineno_stats', callback=(lambda x: write_statistics_to_file(x, False)
), connection=_connection)
        create_csv('tracemalloc_lineno_cumulative_stats', callback=(lambda x: write_statistics_to_file(x, True)
), connection=_connection)

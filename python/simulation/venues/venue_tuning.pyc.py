...
class Venue(metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.VENUE)):
    ...
    def __init__(self, source_venue_type=None, **kwargs):
        ...
        if source_venue_type is None or not source_venue_type.variable_venues is None or source_venue_type.variable_venues.enable_civic_policy_support:
            self._civic_policy_provider = None
    ...
    def _try_start_special_event(self, scheduler, alarm_data, extra_data):
        ...
        if self._active_special_event_id is None:
            ...
            if situation.venue_invitation_message is not None:
    ...
    def _try_balance_club_gatherings(self, scheduler, alarm_data, extra_data=None):
        ...
        for club in clubs:
            ...
            if not club.is_gathering_auto_spawning_available():
                continue
            if lot_household is not None:
                if not any((member.household is lot_household for member in club.members)):
                    continue
 ...
 L.1280         4  SETUP_LOOP           50  'to 50'
               ...
               10  GET_ITER         
             12_0  COME_FROM            46  '46'
             12_1  COME_FROM            44  '44'
 ...
 L.1284        36  LOAD_FAST                'num_objects'
               ...
               40  LOAD_ATTR                number
               42  COMPARE_OP               <
               44  POP_JUMP_IF_FALSE_LOOP    12  'to 12'

 L.1287        46  JUMP_LOOP            12  'to 12'
               48  POP_BLOCK        
...
Parse error at or near `JUMP_LOOP' instruction at offset 46

...
# Compiled at: 2020-09-25 11:51:10
...
class WaitInLineSuperInteraction(SitOrStandSuperInteraction):
    ...
    def _initialize_line_component_on_target(self):
        ...
        return (
         target.waiting_line_component.join_line(self), target.waiting_line_component)
    ...
    def may_reserve_on_stored_target(self):
        ...
        for target in targets:
            ...
            if target_reservation_handler.may_reserve():
                if target.supports_affordance(self._stored_aop):
                    return True
    ...
    def get_adjustment_constraint(cls, inst, sim=DEFAULT, participant_type=ParticipantType.Actor, **kwargs):
        ...
        if not inst._waiting_line.is_first_in_line(inst):
            if not sim_in_front_of_me is not None or interaction_in_front_of_me._instanced_stored_interaction is not None:

...
def generate_privacy_management_data(filter=None):
    ...
    for instance in privacy_service._privacy_instances:
        ...
        if filter_list is None or ACTIVE_FILTER not in filter_list:
            if instance.is_active:
                continue
...
def populate_violators(entry, instance, active_instances):
    ...
    privacy_instance_to_violators = {instance: violators for instance, violators in  if instance in active_instances}
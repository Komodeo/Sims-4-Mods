...
class FollowPath(distributor.ops.ElementDistributionOpMixin, elements.SubclassableGeneratorElement):
    ...
    def _run_gen(self, timeline):
        ...
        if self.actor.is_sim:
            ...
            if teleport_routing is not None:
                if not override_teleport:
                    if not routing_slave_prevents_teleport:
                                ...
                                if self.actor.is_sim:
                                    ...
                                    while True:
                                        ...
                                        if self.canceled:
                                            break
                                        if self.finished:
                                            if not self.actor.routing_component.route_event_context.has_scheduled_events() or current_time > self._animation_sleep_end:
                                                break
                                        else:
    ...
    def _run_gen(self, timeline):
        ...
        try:
            ...
            if goals:
                ...
                if not effective_ghost_route or teleport_style_data is not None:
                    ...
                    if not connectivity_result:
                        ...
                        force_ghost = True
                if ocean_wading_interval is not None and not bool(build_buy.get_pond_id(target_location.transform.translation)):
                    ...
                    if not effective_ghost_route:
                        if teleport_style_data is not None:
                            ...
                            if path_final_location is not None:
                                ...
                                if sim_target_distance < teleport_style_data.teleport_min_distance:
                                    if self.path.length_squared() > teleport_style_data.teleport_min_distance:
...
def get_fgl_context_for_jig_definition(jig_definition, sim, target_sim=None, ignore_sim=True, max_dist=None, height_tolerance=None, stay_outside=False, stay_in_connectivity_group=True, ignore_restrictions=False, fallback_routing_surface=None, object_id=None, participant_to_face=None, facing_radius=None, stay_on_world=False, use_intended_location=True, model_suite_state_index=None, force_pool_surface_water_depth=None, min_water_depth=None, max_water_depth=None, fallback_starting_position=None, fallback_min_water_depth=None, fallback_max_water_depth=None, stay_in_lot=False, height_clearance_override=None, max_pond_water_depth=None, stay_in_enclosed_room=False):
    ...
    if use_intended_location:
        ...
        if reference_routing_surface.type != relative_obj.routing_surface.type and not placement.surface_supports_object_placement(reference_routing_surface, jig_definition.id):

...
# Compiled at: 2021-06-08 17:53:58
...
class PeriodicStatisticChangeElement(HasTunableFactory, elements.SubclassableGeneratorElement):
    ...
    def _add_operation_if_valid(self, resolver, loot_op, periodic_mods_by_participant, exclusive_mods_by_participant, skip_test=False):
        ...
        if not stat is None or stat.continuous:
            if is_dynamic_skill_loot_op or isinstance(loot_op, StatisticChangeOp):
    ...
    def __repr__(self):
        ...
        for condition in self.conditions:
            if callable(condition):
                if not isinstance(condition, TunableFactory.TunableFactoryWrapper):
...
class StatisticDecayByCategory(elements.ParentElement):
    ...
    def _run(self, timeline):
        return timeline.run_child(build_critical_section_with_finally(lambda _: self._start_decay_effects()
, self._sequence, lambda _: self._end_decay_effects()
))

...
# Compiled at: 2017-12-08 17:46:19
    ...
    def compatible_with_current_posture_state(self, sim):
                                                ...
                                                if sim.posture.target in list(target_sim.posture.target.adjacent_parts_gen()):
                                                    if sim_posture_state_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid:
                                                        if target_posture_state_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid:
                                                            return True
                    if sim_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid:
                        if self.target is not None:
                            if self.target.is_sim:
                                if sim_posture_state_constraint.intersect(SIT_INTIMATE_CONSTRAINT).valid:
        ...
        if not sim.posture.mobile:
            ...
            if not sim_intersection.valid:
                return False
            if not self.affordance.is_social and self.target is not None or self.target.is_sim:

...
class SuperInteraction(RouteEventProviderMixin, Interaction, StageControllerElement):
        ...
        for affordance in cls.all_affordances_gen():
            if affordance.allow_user_directed:
                ...
                if cls.target_type == TargetType.ACTOR:
                    if cls.target_type != affordance.target_type:
    ...
    def _constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor, **kwargs):
        ...
        if inst is not None:
            if inst.sim.posture.source_interaction is inst:
                ...
                if target is not None:
                    if target.additional_interaction_constraints is not None:
                        for tuned_additional_constraint in target.additional_interaction_constraints:
                            ...
                            found_constraint = True
                            if not affordance_links is None:
                                if affordance_links(cls):
                                    pass
    ...
    def _estimate_distance_cache_key(self):
        ...
        all_potentially_included_sis = frozenset(((si, si.is_guaranteed) for sim in required_sims for si in sim.si_state if  if not si.is_finishing))
        return (
         self.constraint_intersection(posture_state=None).estimate_distance_cache_key(),
        ...
        if carry_participant is not None and carry_participant != ParticipantType.CreatedObject:
            ...
            if carry_target is not None:
                if self.context.carry_target is None:
                    ...
                    if not self.context.carry_target is not None or carry_target != self.context.carry_target:
    ...
    def get_combinable_interactions_with_safe_carryables(self):
        ...
        for combinable in self.combinable_interactions:
            ...
            if combinable_carry is not None:
                if combinable_carry not in combined_carry_targets:
    ...
    def _parameterized_autonomy_helper_gen(self, timeline, commodity_info_list, context_bucket, participant_type=ParticipantType.Actor, fallback_notification=None, push_on_success_or_fail=None, as_continuation=True):
        ...
        for commodity_info in reversed(commodity_info_list):
            ...
            if self.target is not None:
                ...
                elif not commodity_info.consider_same_target:
    ...
    def _process_parameterized_autonomy_request_gen(self, timeline, sim, parameterized_requests, context_bucket, as_continuation=True):
        if self.target is not None:
            if self.target.is_sim:
                preferred_carrying_sim = self.target if self.sim is sim else None
    ...
    def provide_route_events(self, route_event_context, sim, path, failed_types=None, **kwargs):
        ...
        for route_event_tuple in self.route_events:
            ...
            for participant in participants:
                if participant is sim:
                    if self.can_provide_route_event(route_event_tuple.route_event, failed_types, resolver):
                        if not route_event_context.route_event_already_scheduled(route_event_tuple.route_event):
    ...
    def prepare_gen(self, timeline, cancel_incompatible_carry_interactions=False):
        ...
        if self.waiting_line is not None and not self._pushed_waiting_line:
            if self.target is not current_posture_target or self.waiting_line.allow_line_on_same_target:
    ...
    def run_pre_transition_behavior(self):
        ...
        if not target_sim is not None and self.is_user_directed or target_sim.is_npc:
            if target_sim.sim_info.lives_here or self.sim.sim_info.lives_here or services.get_zone_situation_manager().is_player_greeted():
    ...
    def should_replace_posture_source_interaction(new_interaction):
        if new_interaction.simless:
            return False
        if not new_interaction.sim.posture.posture_type is new_interaction.provided_posture_type or new_interaction.sim.posture.source_interaction is not new_interaction:
            if new_interaction.sim.posture.multi_sim and not new_interaction.sim.posture.is_puppet:
                return True
            return False
    ...
    def is_compatible_with_stand_at_none(self):
        ...
        for constraint in interaction_constraint:
            ...
            if posture_state_spec is not None:
                if not posture_state_spec.body_target is PostureSpecVariable.ANYTHING or posture_state_spec.body_target is None:
                    break
                else:
                    break
        else:
    ...
    def get_cancel_replacement_aops_contexts_postures(self, can_transfer_ownership=True, carry_cancel_override=None):
        ...
        if can_transfer_ownership:
            if carry_target is not None:
                if carry_target.parent is self.sim and not self.is_putdown:
                    head_interaction = self.sim.queue.get_head()
                    if head_interaction is not None:
                        if not head_interaction.is_super or head_interaction is not self:
        ...
        for on_cancel_aop, context, posture in on_cancel_aops_contexts_postures:
            ...
            if self in sim.si_state:
                ...
                if posture.has_linked_sim_pushed_cancel(carry_cancel_override=carry_cancel_override):
                    if not context.insert_strategy == QueueInsertStrategy.FIRST:
                        continue
    ...
    def _cancel_eventually(self, *args, immediate=False, **kwargs):
        if not self.is_finishing or self._cancel_deferred is not None:
            if not immediate:
        ...
        try:
            try:
                ...
                if self.must_run:
                    ...
                    if self.sim is not None:
                        if notify_UI:
                            if not self.is_finishing_naturally:
    ...
    def displace(self, displaced_by, cancel_reason_msg=None, **kwargs):
        if displaced_by.source == InteractionContext.SOURCE_BODY_CANCEL_AOP or displaced_by.source == InteractionContext.SOURCE_CARRY_CANCEL_AOP:
            if self.transition is not None:
 ...
 cancel_reason_msg, notify_UI=True, 
    ...
    def get_interaction_locked_params(self, actor_name, target_name):
        ...
        def get_asm_param(skill_stat, skill_sim, skill_actor_name):
            if not skill_sim is None or skill_sim.is_sim:
                ...
                if animation_stat is not None:
                    ...
                    if asm_param_name is not None:
                        if asm_param_value is not None:
                            return {(
 asm_param_name, skill_actor_name): asm_param_value}
                else:
                    if tested_result.stat_asm_param is not None:

...
class SimPickerMixin:
    ...
    def _verify_tuning_callback(cls):
        ...
        if not cls.include_instantiated_sims or cls.include_uninstantiated_sims:
        ...
        for sim_info in sim_infos:
            ...
            if not inst_or_cls.include_actor_sim:
                if sim_info is actor_sim_info:
                    continue
            if not inst_or_cls.include_target_sim:
    ...
    def create_row(cls, inst, tag, select_default=False):
        ...
        if inst_or_cls.requires_age_romance_check:
            ...
            if inst is not None:
                ...
                if actor_sim_info.age <= Age.TEEN and not target_sim_info.age >= Age.YOUNGADULT:
                    if not actor_sim_info.age >= Age.YOUNGADULT or target_sim_info.age <= Age.TEEN:
                        allow_romance = False
...
class LotPickerMixin:
    ...
    def _get_valid_lot_choices(cls, inst, target, context, target_list=None):
        ...
        if target_list is None:
            if target is not None:
                if not target.is_sim or target.household is not None:
...
class MapViewPickerInteraction(LotPickerMixin, PickerSuperInteraction):
    ...
    def _create_dialog(self, owner, target_sim=None, target=None, **kwargs):
        ...
        dialog = self.picker_dialog(owner, title=(lambda *_, **__: self.get_name()
), resolver=(self.get_resolver()), traveling_sims=traveling_sims)
        (self._setup_dialog)(dialog, **kwargs)
        dialog.set_target_sim(target_sim)
        dialog.set_target(target)
        dialog.add_listener(self._on_picker_selected)
        ...
        for obj in object_manager.get_objects_with_filter_gen(inst_or_cls.whitelist_filter):
            if sim_level != obj.level:
                if not inst_or_cls.allow_different_level:
                    continue
                if inst_or_cls.blacklist_filter is not None:

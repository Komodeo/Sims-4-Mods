...
class SocialCompatibilityMixin:
            ...
            for required_sim in self.required_sims():
                ...
                for si in required_sim.si_state.all_guaranteed_si_gen(self.priority, self.group_id):
                    ...
                    if bool(participant_type in ParticipantTypeSingle) != bool(si_participant_type in ParticipantTypeSingle):
                        continue
                    if not required_sim.si_state.are_sis_compatible(self, si, participant_type_a=participant_type, participant_type_b=si_participant_type,
                      for_sim=required_sim):
                    ...
                    if not source_interaction is None:
                        if source_interaction.is_finishing:
                            continue
                        if not required_sim.si_state.are_sis_compatible(self, source_interaction, participant_type_a=participant_type, participant_type_b=si_participant_type,
                          for_sim=required_sim):
                            incompatible_sims.add(required_sim)
                            if required_sim is not self.sim:
                                break
    ...
    def estimate_distance(self):
        ...
        if target is not None:
            ...
            return (
             estimate, posture_change, included_sis)
...
class SocialSuperInteraction(SocialInteractionMixin, SocialCompatibilityMixin, SuperInteraction):
        ...
        if not self.ignores_greetings:
            ...
            if should_play_targeted_greeting:
                    ...
                    if social_group is target_social_group:
                        ...
                        for group_sim in target_social_group:
                            if group_sim is self.sim:
                                continue
                            if not rel_tracker.has_bit(group_sim.sim_info.sim_id, greetings.Greetings.GREETED_RELATIONSHIP_BIT):
                ...
                if should_greet and picked_sim_ids:
                    ...
                    result = False
            if result:
                if not result.interaction.is_finishing:
        ...
        try:
            ...
            if target_sim.intended_location is not None:
                try:
                    ...
                    if not distance_to_intended is not None or distance_to_intended > LOSAndSocialConstraintTuning.maximum_intended_distance_to_route_nearby:
    ...
    def _constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        if not participant_type == ParticipantType.Actor or cls.relocate_main_group:
            if inst is None or inst.pipeline_progress < PipelineProgress.RUNNING:
                for constraint in (super(SuperInteraction, inst_or_cls)._constraint_gen)(sim, target, participant_type=participant_type, **kwargs):
                    yield constraint

                return
            if inst is None or inst.social_group is None or inst.social_group.constraint_initialized:
                for constraint in (super(SuperInteraction, inst_or_cls)._constraint_gen)(sim, target, participant_type=participant_type, **kwargs):
                    yield constraint
            ...
            if not (recipient is None or recipient.is_sim):
                return
            ...
            if fallback_position is None:
                ...
                return
            fallback_constraint = cls._social_group_type.make_constraint_default(initiator, recipient, fallback_position,
              fallback_routing_surface,
              participant_type=relative_participant,
              picked_object=picked_object,
              participant_slot_overrides=(inst_or_cls._social_group_participant_slot_overrides))
        ...
        if participant_type == ParticipantType.TargetSim:
            ...
            if inst is not None:
                ...
                if target_si is not None:
                    if not issubclass(target_si.affordance, SocialPlaceholderSuperInteraction):
                    ...
                    pass
                logger.error('Attempt to get constraint from Social interaction and no constraint exists: {}', inst,
                  owner='maxr')
                yield Anywhere()
                return
        else:
    ...
    def notify_queue_head(self):
        ...
        try:
            ...
            if self.sim.sim_info.get_current_outfit()[0] == OutfitCategory.BATHING:
                if not self.target.is_sim or self.target.sim_info.age <= Age.TEEN:
    ...
    def _get_social_group_for_this_interaction(self):
        ...
        if social_group is None:
            for existing_social_group in services.social_group_manager().objects:
                ...
                if type(existing_social_group) is self._social_group_type:
                    ...
                    if existing_group_target_sim is not None:
        ...
        if not (social_group is not Noneand social_group.has_room_in_group(self.sim) and social_group.has_room_in_group(self.sim) and social_group.has_room_in_group(target_sim)):
            social_group = None
        else:
            if self.sim in social_group:
                if target_sim is not None:
    ...
    def _derail_on_target_sim_posture_change(self, *args, **kwargs):
        ...
        if self.transition is None:
            return
        if target_simin self.transition.get_transitioning_sims() != self.is_target_sim_location_and_posture_valid():
            return
    ...
    def _get_target_aop_and_context(self):
        ...
        target_sim = self.get_participant(ParticipantType.TargetSim)
        if self.initiated:
            if target_affordance is None or target_sim is None:
                return (None, None)
            if self.social_group is not None:
                if self._should_skip_pushing_target_si(target_sim):
                    return (None, None)
 ...
 None, social_group=self.social_group, 
    ...
    def _cancel(self, finishing_type, *args, propagate_cancelation_to_socials=True, **kwargs):
        if (super()._cancel)(finishing_type, *args, **kwargs):
            ...
            if finishing_type == FinishingType.USER_CANCEL:
                if not self._source_social_si is not None or self._source_social_si.sim is not None:
            ...
            if propagate_cancelation_to_socials:
                if self._social_group is not None:
    ...
    def _detach_from_group(self):
        if self.social_group is not None:
            ...
            if self.target is not None:
                if self.target.is_sim:
                    if self._update_social_geometry_if_target_moves:
                        if self._on_target_intended_location_changed_callback in self.target.routing_component.on_intended_location_changed:
    ...
    def _build_pre_elements(self):
        ...
        def do_pre_run_behavior(_):
            ...
            if not self.target is not None or self.target.is_sim:
...
class SocialPlaceholderSuperInteraction(ProxyInteraction):
    ...
    def generate(cls, proxied_affordance):
        ...
        basic_content_kwargs = {key: getattr(basic_content, key) for key in }

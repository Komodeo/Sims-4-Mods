...
# Compiled at: 2018-12-04 12:46:54
...
class TutorialService(Service):
    ...
    def set_unselectable_sim(self, sim_info):
        ...
        if sim_id != self._unselectable_sim_id:
            if sim_id is None:
                ...
                if self._unselectable_sim_id is None:
                    self._unselectable_sim_count = 1
                else:
    ...
    def is_tutorial_running(self):
        drama_scheduler = services.drama_scheduler_service()
        if not (drama_scheduler is None or drama_scheduler.get_running_nodes_by_drama_node_type(DramaNodeType.TUTORIAL)):
            return False
        return True
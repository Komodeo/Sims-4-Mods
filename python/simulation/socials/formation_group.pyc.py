...
# Compiled at: 2021-05-04 10:25:15
...
class FormationSocialGroup(SocialGroup, RouteEventProviderMixin):
    ...
    def _update_formation_offsets(self, path):
        ...
        available_slave_slots.sort(key=(lambda x: x[0].magnitude_squared()
), reverse=True)
        ...
        for group_sim in self:
            for si in group_sim.get_all_running_and_queued_interactions():
                if si.transition is not None:
                    if si.social_group is not self:
            ...
            if target is None:
                ...
                if weighted_events is None:
                    ...
                    for routing_event_tuple in self.routing_events:
                        if routing_event_tuple.actor_event.test(resolver):
                            if routing_event_tuple.target_event.test(target_resolver):

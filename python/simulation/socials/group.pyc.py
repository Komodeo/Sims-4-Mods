...
class SocialGroup(objects.components.ComponentContainer, HasStatisticComponent, metaclass=sims4.tuning.instances.TunedInstanceMetaclass, manager=services.get_instance_manager(resources.Types.SOCIAL_GROUP)):
    ...
    def _get_social_anchor_object(cls, si, target_sim):
        ...
        if si.sim.parent is target_sim or target_sim is not None and target_sim.parent is si.sim:
    ...
    def send_target_update(self, sim, target):
        if not self.is_visible:
            return
        if not (target is None or target.is_sim):
            return
    ...
    def _on_induction_status_updated(self, sim):
        ...
        if registered_si is None:
            return
        if not (sim not in self._induction_map or self._induction_map[sim]):
    ...
    def _can_sim_be_adjusted(self, sim, initial=False):
        ...
        if head is not None:
            if head.is_super:
                return False
        if not initial:
            if sim.queue.transition_in_progress or sim.is_moving:
                return False
    ...
    def _consider_adjusting_sim(self, sim=None, initial=False):
        ...
        if not self.geometry or len(self.geometry) < 2:
            return False
        if sim is None:
            if not self.adjust_sim_positions_dynamically:
                return False
            if sim is not None:
                if sim not in self:
    ...
    def _try_adjusting_constraint(self):
        if not self._anchor_object is not None:
            if len(self) == 0 or self._focus is None:
                return False
            if not self._constraint is not None and self.position is not None or self.geometry:
    ...
    def with_social_focus(self, owner_sim, speaker, required_sims, *args):
        ...
        return build_critical_section_with_finally(lambda _: focus_group.active_focus_begin(owner_sim, active_list)
, args, lambda _: focus_group.active_focus_end(owner_sim, active_list)
)
    ...
    def without_social_focus(self, owner_sim, sim, *args):
        ...
        if sim_focus_entry is None:
            return args
        focus_group = self._focus_group
        return build_critical_section_with_finally(lambda _: focus_group.remove_focus_entry_for_sim(owner_sim, sim, SimFocus.LAYER_SUPER_INTERACTION)
, args, lambda _: focus_group.add_focus_entry_for_sim(owner_sim, sim, SimFocus.LAYER_SUPER_INTERACTION, sim_focus_entry)
)
    ...
    def with_target_focus(self, owner_sim, speaker, target, *args):
        ...
        focus_group = self._focus_group
        return build_critical_section_with_finally(lambda _: focus_group.active_focus_begin(owner_sim, active_list, immediate=True)
, args, lambda _: focus_group.active_focus_end(owner_sim, active_list)
)
    ...
    def with_listener_focus(self, owner_sim, listener, *args):
        ...
        focus_group = self._focus_group
        return build_critical_section_with_finally(lambda _: focus_group.active_focus_begin(owner_sim, active_list, immediate=False)
, args, lambda _: focus_group.active_focus_end(owner_sim, active_list)
)
    ...
    def get_social_animation_element(self, social_animation_ref):
        ...
        return unless(lambda: self._has_played_social_animation
, build_element((set_has_played_social_animation,

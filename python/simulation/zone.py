...
class Zone:
    ...
    def get_zone_info(self):
        ...
        return (
         lot_description_id, world_description_id, neighborhood_id, neighborhood_data)
    ...
    def update_household_objects_ownership(self):
        ...
        if not (venue_instance is None or venue_instance.requires_front_door):
            return
        gameplay_zone_data = zone_data_proto.gameplay_zone_data
        active_household_id = services.active_household_id()
        if self.lot.owner_household_id == 0:
            travel_group = self.travel_group_manager.get_travel_group_by_zone_id(self.id)
            if not travel_group is None:
                if not protocol_buffer_utils.has_field(gameplay_zone_data, 'active_travel_group_id_on_save') or gameplay_zone_data.active_travel_group_id_on_save != travel_group.id:
                    self._set_zone_objects_household_owner_id(None, skip_objects_owned_by_household_ids={active_household_id})
        else:
            if self.lot.owner_household_id == active_household_id:
                if not protocol_buffer_utils.has_field(gameplay_zone_data, 'active_household_id_on_save') or gameplay_zone_data.lot_owner_household_id_on_save != active_household_id:
                    self._set_zone_objects_household_owner_id(active_household_id)
    ...
    def should_restore_sis(self):
        if not self.time_has_passed_in_world_since_zone_save():
            if self.venue_changed_between_save_and_load() or self.lot_owner_household_changed_between_save_and_load() or self.active_household_changed_between_save_and_load() or self.is_first_visit_to_zone:
                return False
            return True
    ...
    def _get_restricted_autonomy_polygon(self):
        ...
        points.sort(key=(lambda k: sims4.math.atan2(k.x - center.x, k.z - center.z)
), reverse=True)

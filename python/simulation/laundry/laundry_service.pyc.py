...
# Compiled at: 2018-07-27 15:22:13
...
class LaundryService(RegisterTestEventMixin, Service):
    ...
    def _add_clothing_pile_to_hamper(self, sim):
        ...
        available_hampers = set((obj for obj in self._hampers if not obj.state_value_active(full_hamper_state) if  if not obj.state_value_active(full_hamper_state)))
    ...
    def on_spin_outfit_change(self, sim, outfit_category_and_index, interaction):
        if not (self._affected_household is None or self.hero_object_exist):
            return
        ...
        if current_outfit not in no_pile_outfits:
            if target_outfit not in no_pile_outfits:
                if not sim.on_home_lot or current_outfit not in naked_outfits:
                    if not self._add_clothing_pile_to_hamper(sim):

...
# Compiled at: 2020-12-02 14:28:38
...
def log_transition_handle(sim, handle, geometry, path, valid, type_str):
    ...
    if transition_log is None:
        return
    entry = {'build_pass':transition_log.current_pass,  'handle':'{} Surface: {}'.format(handle, handle.routing_surface if handle is not None else None), 
...
def log_path_data_to_gsi(sim, interaction=None):
    ...
    if transition_log is None:
        return
    archive_data = {'archive_id':gsi_path_id(),  'sim_name':sim.full_name, 
...
def log_transition_path_automation(sim=None, data=None):
    ...
    output('[AreaInstanceTransitionPath] SimTransPathData;         Interaction:%s,         Path:%s,         PathCost:%s,         DestSpec:%s' % (
     data['interaction'].split()[0],
     pathStr,
     data['pathCost'],
     destStr.replace(',', '')))
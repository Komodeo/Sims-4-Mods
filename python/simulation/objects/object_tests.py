    ...
    def get_all_objects(self, object_manager):
        ...
        if self.test_type == TagTestType.CONTAINS_ANY_TAG_IN_SET:
            ...
            if self.test_type == TagTestType.CONTAINS_ALL_TAGS_IN_SET:
                ...
                if self.test_type == TagTestType.CONTAINS_NO_TAGS_IN_SET:
                    return set((obj for obj in object_manager.values() if not obj.is_sim if not set(obj.get_tags() & self.tag_set) if  if not set(obj.get_tags() & self.tag_set)))
                logger.error('ObjectTagFactory recieved unrecognized TagTestType {}, defaulting to False', (self.test_type), owner='msantander')
        objects_matching_any_tag = set((obj for obj in objects_matching_any_tag if not obj.is_sim if  if not obj.is_sim))
...
class ObjectTrendingFactory(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_all_objects(self, object_manager):
        ...
        return set((obj for obj in objects_matching_any_tag if not obj.is_sim if  if not obj.is_sim))
    ...
    def factory(tested_object, state_set, test_type, object_requirement, **kwargs):
        ...
        if object_requirement is not None:
            if object_requirement.type == BasicStateCheckFactory.TAG_TYPE:
                ...
                if object_requirement.type == BasicStateCheckFactory.DEFINITION_TYPE:
    ...
    def __call__(self, obj=None):
        ...
        if self.test_type is not None:
            ...
            if not self.value_threshold.compare(obj_value):
                return TestResultNumeric(False, 'ObjectPurchasedTest: Incorrect or invalid value object purchased for test: {}, value: {}', obj,
...
class ObjectScoringPreferenceTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        ...
        return {'affordance':ParticipantType.Affordance,  'targets':ParticipantType.Object, 
...
class CraftedWithIngredientsTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, crafted_object=None):
        ...
        if not self.ingredients_used_threshold.compare(num_ingredients_used):
            return TestResult(False, ('CraftedWithIngredientsTest: Item did not use required amount of ingredients.                                         ingredient count needed: {}, ingredients_used: {}, comparison: {}'.format(self.ingredients_used_threshold.value, num_ingredients_used, self.ingredients_used_threshold.comparison)),
...
class ScheduledSituationSpecialObjectTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, target_objects=None, host_sim=None):
        ...
        for object_target in target_objects:
            if self.object_tags:
                if not self.object_tags(object_target):
                    return TestResult(False, ('Object {} did not have the required tags.'.format(object_target.id)),
                      tooltip=(self.tooltip))
        ...
        for drama_node in drama_scheduler.all_nodes_gen():
            ...
            if not situation_seed.special_object_definition_id is None:
                if not situation_seed.host_sim_id != host_sim.sim_id:
        ...
        for household in households:
            ...
            if contents_check is None:
                ...
                if content_check_type == self.ITEM_DEFINITION_TEST:
                    ...
                    if content_check_type == self.PARTICIPANT_TYPE_TEST:
                ...
                for item in inv:
                    ...
                    if content_check_type == self.ITEM_DEFINITION_TEST:
                        if contents_check.definition is not None and item_definition_id == contents_check.definition.id:
                            ...
                            if content_check_type == self.TAGGED_ITEM_TEST:
                                ...
                                if content_check_type == self.PARTICIPANT_TYPE_TEST:
                                    ...
                                    if content_check_type == self.ITEM_STATE_TEST:
                                        ...
                                        if content_check_type == self.SITUATION_SPECIAL_OBJECT_TEST:
                ...
                if self.relationship_status is None:
                    ...
                    if not relationship_value < self.relationship_status.value_interval.lower_bound:
                        if relationship_value > self.relationship_status.value_interval.upper_bound:
                            continue
                if self.can_add_relationship is not None:
                    if self.can_add_relationship != relationship_component._can_add_new_relationship:
                        continue
                passing_objects += 1
...
class CustomNameTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, targets=()):
        for target in targets:
            ...
            elif self.has_custom_name is not None:
                if target.has_custom_name() != self.has_custom_name:
            ...
            if target.is_part:
                ...
                for part in target.parts:
                    if part.subroot_index is None or self.subroot_index is None:
                        ...
                        if part.subroot_index != self.subroot_index:
                            continue
                    if part.children:
                        if not self.negate:
                            found_match = True
    ...
    def __call__(self, subject=None, target=None):
        ...
        if consumable_component is not None:
            ...
            if not threshold.compare(bites_left_in_subject):
                return TestResult(False, 'Object {} is expected to have {} {} bites left, but actually has {} bites left.',
                  subject,
                ...
                if sim_info.get_sim_instance(allow_hidden_flags=ALL_HIDDEN_REASONS) is None:
                    ...
                    for si in sim.si_state:
                        ...
                        if posture_target is not None:
                            if posture_target.game_component is not None:
        ...
        for obj in objs:
            ...
            for target in test_targets:
                ...
                if self.test_household_owner:
                    ...
                    if self.consider_renter_as_household_owner:
                        if not owner_id or target_sim.is_renting_zone(current_zone_id):
                            owner_id = sim_id
                else:
...
class ObjectCriteriaTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.desired_state_threshold is not None:
            ...
            if not (threshold_value.state is None or hasattr(threshold_value, 'value')):
                logger.error('invalid state value in desired state threshold for {}: {}', source, tunable_name)

    FACTORY_TUNABLES = {'verify_tunable_callback':_verify_tunable_callback,  'subject_specific_tests':TunableVariant(all_objects=TunableTuple(locked_args={'subject_type': ALL_OBJECTS},
    ...
    def object_meets_criteria(self, obj, active_household_id, active_sim_id, current_zone, positional_relationship_participants=None):
        ...
        if self.on_active_lot:
            if not obj.is_on_active_lot():
                return False
            if self.positional_relationship_test is not None:
                if not self.positional_relationship_test.evaluate(obj, participants=positional_relationship_participants):
                    return False
                if not self.completed or obj.crafting_component is not None:

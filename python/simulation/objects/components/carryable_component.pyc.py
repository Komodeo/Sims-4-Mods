...
class PutDownLiability(Liability):
    ...
    def release(self):
        ...
        if target_object.parent is not self._parent:
            ...
            return
        if carry_component.attempted_putdown:
            if not carry_component.attempted_alternative_putdown or target_object.transient:
            ...
            if not CarryingObject.snap_to_good_location_on_floor(target_object):
                ...
                if not (sim.household.id is not target_object.get_household_owner_id() or sim.inventory_component.player_try_add_object(target_object)):
...
class CarryableComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=objects.components.types.CARRYABLE_COMPONENT):
    ...
    def _get_terrain_transform(self, interaction):
        ...
        if position is not None:
            ...
            return (
             put_down_transform, starting_routing_surface)
        return (None, None)

              ...
              reverse=True):
                ...
                if trait_tracker.has_trait(trait):
                    if replacement_buff_type.can_add(self.owner):
                        ...
                        return (
                         replacement_buff_type, replacement_buff_reason)
...
, True)
    ...
    def remove_buff_entry(self, buff_entry, on_destroy=False):
        if buff_entry is not None:
            if buff_entry.commodity is not None:
                ...
                if not commodity_inst is not None or commodity_inst.state_backed:
                    if not on_destroy:
    ...
    def get_teleport_cost(self, teleport_data, active_multiplier):
        ...
        if cost_tuning is not None:
            ...
            if cost_tuning.cost_is_additive:
                ...
                if current_value - current_cost > cost_tuning.teleport_statistic.min_value:
                    return current_cost
    ...
    def get_teleport_data_and_cost(self, teleport_style, active_multiplier):
        ...
        return (
         teleport_data, self.get_teleport_cost(teleport_data, active_multiplier))
    ...
    def _remove_non_persist_buffs(self):
        buff_types_to_remove = set()
        for buff_type, buff in self._active_buffs.items():
            if not buff.commodity is None or buff.commodity.persisted:
        ...
        if cls.is_add:
            if cls.buffs:
                for buff_info in cls.buffs:
                    yield (
                     buff_info.buff, buff_info.buff_name, buff_info.buff_icon, buff_info.buff_description)

            else:
                buff_manager = services.get_instance_manager(sims4.resources.Types.BUFF)
                for buff_type in buff_manager.types.values():
                    yield (
                     buff_type, buff_type.buff_name, buff_type.icon, buff_type.buff_description)
            ...
            if inst_or_cls.is_add:
                if single_sim:
                    ...
                    if not is_enable:
                        ...
                        if inst_or_cls.picker_dialog.factory == UiObjectPicker:

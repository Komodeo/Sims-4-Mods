...
class GameObject(ClientObjectMixin, ReservationMixin, ScriptObject, reset.ResettableObjectMixin):
    ...
    def _verify_tuning_callback(cls):
        if cls._provided_mobile_posture is not None:
            for affordance in cls._provided_mobile_posture.affordances:
                ...
                if affordance in PostureGraphService.POSTURE_PROVIDING_AFFORDANCES:
    ...
    def is_surface(self, include_parts=False, ignore_deco_slots=False):
        ...
        if inventory_component is not None:
            if inventory_component.has_get_put:
                self._is_surface[key] = True
                return True

        def is_valid_surface_slot(slot_type):
            if ignore_deco_slots and not slot_type.is_deco_slot:
    ...
    def get_gsi_portal_items(self, key_name, value_name):
        ...
        return [
         {key_name: 'Household Owner', value_name: name}]

...
# Compiled at: 2020-07-17 12:07:03
...
, 
    ...
    def on_zone_unload(self):
        ...
        for club in self.clubs:
            ...
            if pc_count > 0:
                npc_count = sum((1 for sim_info in club.members if not sim_info.is_selectable if  if not sim_info.is_selectable))
    ...
    def _does_rule_pass_tests(self, sim_info, rule, aop_or_interaction):
        if aop_or_interaction.target is not None:
            if aop_or_interaction.target.is_sim:
                ...
                if not rule.with_whom is not None or target_sim_info is not sim_info:
    ...
    def generate_members_for_membership_criteria(self, number_of_members, admission_criteria):
        ...
        def _is_valid_member(sim_info):
            ...
            if not sim_info.is_human:
                return False
            if not all((criteria.test_sim_info(sim_info) for criteria in admission_criteria)):
                return False
            return True

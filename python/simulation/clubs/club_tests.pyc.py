...
# Compiled at: 2018-11-15 15:08:57
            ...
            if gathering is not None:
                ...
                if self.subject_club_gathering_status is not None:
                    for subject in test_subjects:
                        ...
                        if sim_inst is None or sim_inst not in gathering_members:
...
class ClubTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def _club_test_enabled(self):
        if not self.recent_member_status is not None:
            if self.room_for_new_members is not None or self.invite_only is not None or self.subject_passes_membership_criteria is not None or self.subject_relationship_with_leader is not None or self.required_sim_count or self.affordance_rule:
                return True
            return False
            ...
            if self.room_for_new_members:
                ...
                if self.subject_passes_membership_criteria is not None:
                    ...
                    if subject_result:
                        ...
                        pass
                    if self.affordance_rule == self.AFFORDANCE_RULE_DISCOURAGED:
                        if rule_status != ClubRuleEncouragementStatus.DISCOURAGED:
                            return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                              tooltip=(self.tooltip))
                    else:
                        if self.affordance_rule == self.AFFORDANCE_RULE_NOT_ENCOURAGED:
                            if rule_status == ClubRuleEncouragementStatus.ENCOURAGED:
                                return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                                  tooltip=(self.tooltip))
                        else:
                            if self.affordance_rule == self.AFFORDANCE_RULE_NOT_DISCOURAGED:
                                if rule_status == ClubRuleEncouragementStatus.DISCOURAGED:
                                    return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                                      tooltip=(self.tooltip))
            return TestResult.TRUE
                    ...
                    if self._club_test_enabled():
                        if common_test_subjects:
                            ...
                            if not any((self._test_club(subject, club, affordance_data=affordance_data) for club in clubs)):
                                return TestResult(False, 'Subject {} fails Club test for {}', subject,

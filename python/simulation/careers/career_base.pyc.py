...
# Compiled at: 2022-07-21 18:49:29
...
class CareerBase(CoworkerMixin, CareerKnowledgeMixin, sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    def get_is_quittable_shift(self, schedule_shift_type=CareerShiftType.ALL_DAY, career_category=None):
        if not career_category == CareerCategory.TeenSideHustle:
            if self.career_category == CareerCategory.TeenPartTime:
                return True
        if not schedule_shift_type == self._current_shift_type:
            if schedule_shift_type == CareerShiftType.ALL_DAY or self._current_shift_type == CareerShiftType.ALL_DAY:
                return True
            return False
    ...
    def is_career_available(cls, inst, sim_info=DEFAULT, from_join=False):
        if inst is not None:
            ...
            if not from_join or inst_or_cls.career_category != CareerCategory.TeenSideHustle:
    ...
    def create_work_state_msg(self):
        ...
        if self.is_work_time:
            ...
            if self.career_panel_type == CareerPanelType.UNIVERSITY_COURSE and not self._sim_info.is_in_travel_group():
                degree_tracker = self._sim_info.degree_tracker
                if not degree_tracker is not None or degree_tracker.allowed_to_enroll:
                ...
                if self.career_panel_type == CareerPanelType.FREELANCE_CAREER:
                    ...
                    if self.requested_day_off and self.requested_day_off_reason != career_ops.CareerTimeOffReason.WORK_FROM_HOME:
        ...
        if self.on_assignment:
            if self._assignment_late:
                ...
                if self.has_completed_active_assignments:
                    ...
                    if not self.has_attended_first_day:
                        msg.work_state = DistributorOps_pb2.SetAtWorkInfo.WORKDAY_OVER
                    else:
    ...
    def get_career_entry_level(cls, career_history=None, career_track=None, resolver=None):
        ...
        if not histories or use_start_track:
        ...
        if is_load:
            ...
            if allow_outfit_generation:
                if self.has_outfit():
                    ...
                    if not (work_outfit.generate_on_level_change or is_level_change):
    ...
    def career_stop(self, for_travel=False, is_level_change=False):
        if self._work_scheduler is not None:
            ...
            if not for_travel:
                self._work_scheduler = None
            if self._current_gigs:
                if not for_travel:
                    if not is_level_change:
                        self.clear_all_gigs()
                if self._work_scheduler_override is not None:
                    self._work_scheduler_override.destroy()
                    self._work_scheduler_override = for_travel or None
        ...
        if self._requested_day_off_reason == CareerTimeOffReason.NO_TIME_OFF:
            ...
            if self.on_assignment:
                ...
                if evaluation is not None:
                    if evaluation != Evaluation.ON_TARGET:
    ...
    def _start_work_callback(self, scheduler, alarm_data, extra_data):
        ...
        if self.requested_day_off_reason == CareerTimeOffReason.WORK_FROM_HOME:
            ...
            def get_situation_jobs(sim):
                ...
                return {situation.get_current_job_for_sim(sim) for situation in }
    ...
    def _try_offer_career_event(self):
        ...
        if not self.allow_active_offlot:
            ...
            if self._sim_info in _career_event_overrides:
                ...
                if current_gig is not None and current_gig.career_events:
                    career_event = current_gig.get_random_gig_event()
                else:
                    self._prune_stale_career_event_cooldowns()
                    resolver = SingleSimResolver(self._sim_info)
                    available_events = tuple((event for event in self.career_events if not self.is_career_event_on_cooldown(event) if event.tests.run_tests(resolver) if  if not self.is_career_event_on_cooldown(event) if event.tests.run_tests(resolver)))
                ...
                if current_career is not None:
                    if self.career_event_manager.is_same_current_event(current_career):
                        ...
                        if current_career_event is not None:
                            if not current_career_event.end_of_day_results_reported:
    ...
    def _prune_stale_career_event_cooldowns(self):
        current_day = self.days_worked_statistic.get_value()
        self._career_event_cooldown_map = {event_id: day for event_id, day in  if current_day < day}
    ...
    def _try_invite_over(self):
        if self.invite_over is None:
            return
        if not (self.sim_info.is_npc or self.sim_info.is_at_home):
            return
    ...
    def handle_career_loot(self, hours_worked, left_early=False):
        ...
        gig = self.get_current_gig()
        if not gig and left_early or gig.odd_job_tuning is not None:

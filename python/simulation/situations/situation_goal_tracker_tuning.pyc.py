...
class TunableSituationGoalTracker(TunableFactory):
    ...
    def _get_situation_goal_tracker(situation=None):
        return (
         GoalTrackerType.STANDARD_GOAL_TRACKER, None if situation is None else SituationGoalTracker(situation))
...
class TunableDynamicSituationGoalTracker(TunableFactory):
    ...
    def _get_dynamic_goal_tracker(situation=None):
        return (
         GoalTrackerType.DYNAMIC_GOAL_TRACKER, None if situation is None else DynamicSituationGoalTracker(situation))
...
class TunableSimpleSituationGoalTracker(TunableFactory):
    ...
    def _get_simple_goal_tracker(situation=None):
        return (
         GoalTrackerType.SIMPLE_GOAL_TRACKER, None if situation is None else SimpleSituationGoalTracker(situation))

...
class SituationJobReward(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _apply_loot(self, sim, situation):
        if self.loot_target_choice == JobLootTargetChoice.RANDOM_SIM:
            ...
            if self.loot_target_choice == JobLootTargetChoice.ALL_SIMS_IN_JOB:
                if self.loot_target is not None:
...
class SituationJob(HasDependentTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)):
    ...
    def get_score(cls, event, resolver, **kwargs):
        if event == event_testing.test_variants.TestEvent.InteractionComplete:
                ...
                if score_list.role_tags is not None:
                    if target is None:
                        ...
                        for situation in services.get_zone_situation_manager().get_situations_sim_is_in(target):
                            ...
                            if score_list.role_tags.whitelist & situation.get_role_tags_for_sim(target):
                                should_apply_score = True
            ...
            if event == event_testing.test_variants.TestEvent.ItemCrafted:

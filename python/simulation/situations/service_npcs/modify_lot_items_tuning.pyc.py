            ...
            if object_criteria is not None:
                ...
                for action, tests in modifications.copy().items():
                    ...
                    if action_type == ModifyAllLotItems.DESTROY_OBJECT:
                        ...
                        break
                    if action_type == ModifyAllLotItems.SET_STATE:
                        ...
                        if obj.state_component and obj.has_state(new_state_value.state):
                            ...
                            if action_type in (ModifyAllLotItems.INVENTORY_TRANSFER,
                             ModifyAllLotItems.DELIVER_BILLS):
                                element = action.action_value()
                                element._do_behavior()
                            else:
                                if action_type == ModifyAllLotItems.SET_ON_FIRE:
                                    fire_service = services.get_fire_service()
                                    fire_service.spawn_fire_at_object(obj)
                                else:
                                    if action_type == ModifyAllLotItems.CLEANUP_VEHICLE:

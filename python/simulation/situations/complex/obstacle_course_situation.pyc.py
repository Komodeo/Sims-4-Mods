...
class RunCourseState(CommonInteractionCompletedSituationState):
    ...
    def _additional_tests(self, sim_info, event, resolver):
        ...
        if not (athlete is None or sim_info.id == athlete.id):
            return False
        return True
...
class ObstacleCourseSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(0, WaitForSimJobsState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.coach_job_and_role_state.job, cls.coach_job_and_role_state.role_state),
         (
    ...
    def _on_add_sim_to_situation(self, sim, job_type, *args, **kwargs):
        ...
        if self.get_coach() is not None:
            if self.get_athlete() is not None:
                object_manager = services.object_manager()
                obstacles = {object_manager.get(obstacle_id) for obstacle_id in }

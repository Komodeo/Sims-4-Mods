...
class FinishWalkState(CommonInteractionCompletedSituationState):
    ...
    def _additional_tests(self, sim_info, event, resolver):
        walker = self.owner.get_walker()
        if not (walker is None or sim_info.id == walker.id):
            return False
        return True
...
class WalkDogSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(0, WalkState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.walker_job_and_role_state.job, cls.walker_job_and_role_state.role_state),
         (
    ...
    def get_walk_nodes(cls):
        ...
        if not attractor_objects:
            return (
             get_objects(cls.attractor_point_tags.fallback_tags), True)
    ...
    def _build_walking_path(self):
        ...
        min_dist_obj = min(all_obj_and_pos_list, key=(lambda k: (k[1] - sim_position).magnitude_2d_squared()
))[0]
        obj_and_pos_list = []
        if not is_fallback:
            tags = min_dist_obj.get_tags()
            matching_tags = {tag for tag in  if tag in tags}
        ...
        obj_and_pos_list.sort(key=(lambda k: sims4.math.atan2(k[1].x - center.x, k[1].z - center.z)
), reverse=True)
        start_index = 0

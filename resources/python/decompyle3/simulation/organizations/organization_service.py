...
class OrganizationService(Service):
    ...
    def validate_venue_event(self, drama_node):
        ...
        if venue_tuning is not self.get_organization_venue_tuning(type(drama_node)):
            ...
            alarm_handle = drama_node.schedule_duration_alarm((lambda *_: self._schedule_cancelled_organization_event(drama_node.guid64)
), cross_zone=True)
    ...
    def generate_organization_members(self, org_id, amount=0, blacklist_sims=(), additional_filter_terms=None, minimum=None):
        ...
        if minimum is not None:
            members_needed = minimum - len(member_result_ids) if len(member_result_ids) <= minimum else amount - len(member_result_ids)
            if members_needed > 0:

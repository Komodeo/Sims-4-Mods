...
class GameSpeedLiability(Liability, HasTunableFactory):
        ...
        if self.new_game_speed == ClockSpeedMode.SUPER_SPEED3:
            ...
            def validity_check(situations_to_ignore=None):
                ...
                if current_venue is not None:
                    if not current_venue.allow_super_speed_three:
                        return False
                    for situation in situation_manager.get_all():
                        if situations_to_ignore:
                            if situation in situations_to_ignore:
                                continue
                        if situation.blocks_super_speed_three:
                            return False
                        if situation.park_interactions_on_SS3:
                            for situation_sim in situation.all_sims_in_situation_gen():
                                if not situation_sim.is_npc:
                                    continue
                                else:
                                    run_affordance = random.choice(situation.park_interactions_on_SS3)
                                    self.special_case_sims[situation_sim] = run_affordance
                    ...
                    def validate_household_sim(test_sim):
                        ...
                        if test_sim.is_selectable:
                            ...
                            if roommate_service is not None:
                                if roommate_service.get_home_zone_id(test_sim.sim_id) == current_zone_id:
                                    run_affordance = roommate_service.get_ss3_affordance()
                                    if run_affordance is not None:
                                        self.special_case_sims[test_sim] = run_affordance
                                        return True
                            travel_group = test_sim.travel_group
                            if travel_group is not None:
                                if travel_group.zone_id == current_zone_id:
                                    if travel_group.is_active_sim_in_travel_group:
                                        run_affordance = random.choice(TravelGroupTuning.SS3_PARK_INTERACTIONS)
                                        if run_affordance is not None:
                                            self.special_case_sims[test_sim] = run_affordance
                                            return True
                        return False
            ...
            for situation in situation_manager.running_situations():
                ...
                if not situation.super_speed3_replacement_speed:
                    ...
                    replacement_request = services.game_clock_service().push_speed((situation.super_speed3_replacement_speed),
                      validity_check=(lambda s=situation: s in situation_manager.running_situations() and validity_check(situations_to_ignore=(s,))
),
                      reason='Situation alternate SS3 speed request')
                    situation_id = situation.id
                    callback_fn = functools.partial(situation_ended_callback, request=replacement_request,
                      situation_speed_requests=(self.situation_speed_request_data))
            ...
            validity_check = None
        self.speed_request = services.game_clock_service().push_speed((self.new_game_speed),
          validity_check=validity_check, reason='SS3 interaction')
    ...
    def _cancel_ss3_park_affordance(self, affected_sim, affordance):
        for run_affordance in {affordance.affordance}(affordance.affordances):
...
class PushAffordanceOnRouteFailLiability(Liability, HasTunableFactory, AutoFactoryInit):
    ...
    def release(self):
        if self._interaction.transition_failed or self.push_affordance_on_cancel and self._interaction.has_been_canceled:
...
class ServiceNpcRequest(XevtTriggeredElement):
    ...
    def _do_behavior(self):
        ...
        if request_type == self.HIRE:
            ...
            if not self.notification is not None or finishing_time is not None:
        ...
        for arg in self.arguments:
            if arg.arg_type == self.ARG_TYPE_PARTICIPANT:
                ...
                if arg.arg_type == self.ARG_TYPE_LITERAL:
                    ...
                    if arg.arg_type == self.ARG_TYPE_TAG:

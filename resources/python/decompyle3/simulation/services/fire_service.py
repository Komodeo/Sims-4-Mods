...
class FireService(Service):
            ...
            if saver_sim_info.can_live_alone:
                ...
                for toddler in self._toddlers_to_save:
                    if toddler.household is None:
                        continue
                    elif toddler.household is saver_sim_info.household:
                        return True
    ...
    def is_fire_allowed(self, transform, routing_surface, run_placement_tests=True):
        ...
        if not (venue is None or venue.allows_fire):
            logger.info("Trying to spawn a fire on a venue that doesn't allow fire.")
            return False
        if run_placement_tests:
            if not self._placement_tests(transform.translation, routing_surface):
    ...
    def _placement_tests(self, new_position, surface_id=None, fire_object=None):
        ...
        if surface_id is not None:
            if build_buy.has_floor_at_location(new_position, surface_id.secondary_id):
                if build_buy.is_location_pool(new_position, surface_id.secondary_id) or build_buy.is_location_pool(new_position, surface_id.secondary_id + 1):

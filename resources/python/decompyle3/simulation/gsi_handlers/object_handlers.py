...
def generate_object_manager_data(*args, zone_id: int=None, filter=None, **kwargs):
    ...
    for cur_obj in list(itertools.chain(zone.object_manager.objects, zone.prop_manager.objects, zone.inventory_manager.objects)):
        ...
        if not filter_list is None:
            if not ('sim_objects' in filter_list and cur_obj.is_sim):
                if not ('inventory' in filter_list and cur_obj.is_in_inventory()):
                    if not ('prototype_objects' in filter_list and class_str == 'prototype'):
                        if not ('game_objects' in filter_list and class_str != 'prototype'):
                            if not 'open_street' in filter_list or on_active_lot:

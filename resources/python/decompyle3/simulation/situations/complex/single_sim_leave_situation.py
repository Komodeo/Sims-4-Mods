...
class SingleSimLeaveSituation(situations.situation_complex.SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, LeaveState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.situation_role.situation_job, cls.situation_role.leave_role_state)]
...
class LeaveState(situations.situation_complex.SituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionExitedPipeline:
            if not self.owner._leaver is not None and sim_info is self.owner._leaver.sim_info or resolver(self.owner.affordances_to_monitor):

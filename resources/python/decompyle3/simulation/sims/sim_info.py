...
class SimInfo(SimInfoWithOccultTracker, SimInfoCreationSource.SimInfoCreationSourceMixin, AgingMixin, PregnancyClientMixin, SimInfoFavoriteMixin, ComponentContainer, HasStatisticComponent):
    ...
    def get_is_enabled_in_skewer(self, consider_active_sim=True):
        ...
        if self.is_pet:
            ...
            if tutorial_service is not None:
                if tutorial_service.is_sim_unselectable(self):
                    return False
            return True
 ...
 None, away_action=away_action, 
    ...
    def set_zone_on_spawn(self):
        ...
        if self.is_npc:
            if (self._serialization_option == SimSerializationOption.UNDECLARED or self._serialization_option) == SimSerializationOption.LOT and not self._zone_id != current_zone_id:
    ...
    def create_sim_instance(self, position, sim_spawner_tags=None, saved_spawner_tags=None, spawn_action=None, sim_location=None, additional_fgl_search_flags=None, from_load=False, use_fgl=True, spawn_point_override=None, pre_add_fn=None, spawn_at_lot=True, use_random_sim_spawner_tag=True):
        ...
        def init(obj):
            ...
            try:
                ...
                if sim_location is not None:
                    ...
                    if not start_routing_surface.type == routing.SurfaceType.SURFACETYPE_WORLD or start_routing_surface.primary_id != sim_info.zone_id:
        ...
        sim_inst.on_start_up.append((lambda _: sim_inst.fade_in()
) if spawn_action is None else spawn_action)
    ...
    def deploy_vehicle_from_travel(self, sim):
        ...
        if vehicle is not None:
            if sim.inventory_component.try_remove_object_by_id(vehicle.id):
    ...
    def assign_to_travel_group(self, travel_group):
        if self._travel_group_id != 0:
            if not (travel_group.id != self._travel_group_id or game_services.service_manager.is_traveling):
    ...
    def set_freeze_fame(self, should_freeze, force=False):
        if not force:
            if should_freeze:
                ...
                if not self.get_gameplay_option(SimInfoGameplayOptions.FREEZE_FAME):
                    return
    ...
    def _get_serialization_option(self):
        ...
        if not sim.is_selectable:
            if owning_household is not None and self in owning_household or self.is_renting_zone(current_zone_id):
    ...
    def _get_startup_location(self):
        current_zone = services.current_zone()
        if not self._transform_on_load is not None or self._level_on_load is not None:
            ...
            for career in self.careers.values():
                ...
                if start_time_ticks is not None and end_time_ticks is not None:
                    for busy_start_time, busy_end_time in busy_times:
                        if start_time_ticks <= busy_end_time:
                            if end_time_ticks >= busy_start_time:
                                return (
                                 True, career)

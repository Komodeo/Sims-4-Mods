...
# Compiled at: 2021-03-08 17:50:10
...
class UIManager:
    ...
    def _add_interaction_info_to_queue(self, interaction_queue, int_info, interaction, interaction_id_to_insert_after):
        ...
        if not queue_len == 0 or int_info_is_super:
            if int_info.insert_strategy == interactions.context.QueueInsertStrategy.LAST or not int_info_is_super or interaction.should_insert_in_queue_on_append():
                interaction_queue.append(int_info)
            else:
                if int_info.insert_strategy != interactions.context.QueueInsertStrategy.LAST:
                    index_to_add = 0

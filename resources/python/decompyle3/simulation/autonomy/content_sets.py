...
# Compiled at: 2020-05-08 10:27:50
...
def aop_valid_for_scoring(aop, affordance, target, context, include_failed_aops_with_tooltip, considered=None, aop_pre_tests=None):
    ...
    if test_result:
        return test_result
    if include_failed_aops_with_tooltip:
                    ...
                    if for_pie_menu:
                        if not affordance.allow_user_directed:
                            if gsi_considered is not None:
                                ...
                                continue
                        if mixer_interaction_group is not DEFAULT:
                            if affordance.sub_action.mixer_group != mixer_interaction_group:
                                continue
                        targets = _test_affordance_and_get_targets(affordance, super_interaction, potential_targets, sim,
                          considered=gsi_considered)
                        if targets is None:
                            continue
                        if not include_failed_aops_with_tooltip or len(targets) >= MINIMUM_TARGETS_TO_TEST_AFFORDANCE_EARLY:

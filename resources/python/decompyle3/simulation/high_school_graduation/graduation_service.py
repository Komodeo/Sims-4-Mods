...
class GraduationService(Service):
    ...
    def add_sim_info_as_graduating(self, sim_info):
        ...
        if not (time_to_next_graduation == self.DRAMA_NODE_RUNNING and sim_info.is_npc):
            if not time_to_next_graduation != MAX_UINT32 or time_to_next_graduation.in_minutes() >= GraduationService.MIN_TIME_REQUIRED_BEFORE_GRADUATION:

...
class DistributableObjectManager(IndexedManager):
    ...
    def remove(self, obj, **kwargs):
        ...
        if not obj.visible_to_client or zone is not None:
        ...
        for game_object in self._objects.values():
            if self._should_save_object_on_lot(game_object):
                ...
                if zone_data is None:
                    continue
                if not save_result.owner_id != 0 or game_object.get_lost_and_found_registration_info() is None:
                    ...
                    if not (game_object.definition.has_build_buy_tag)(*self._all_bed_tags):
                        ...
                        if (game_object.definition.has_build_buy_tag)(*self.BED_TAGS.kid_beds):
                            total_beds += 1
                            kid_bed_exist = True
                        else:

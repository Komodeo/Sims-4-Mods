    ...
    def get_participants(self, participant_type, **kwargs):
        ...
        if participant_type == ParticipantType.Actor:
            ...
            if participant_type == ParticipantType.ObjectIngredients:
                if self.target is not None:
                    if self.target.crafting_component:
            ...
            if participant_type == ParticipantType.ActorPostureTarget:
                ...
                if participant_type == ParticipantType.AssociatedClub or participant_type == ParticipantType.AssociatedClubLeader or participant_type == ParticipantType.AssociatedClubMembers:
        ...
        if self.interaction is not None:
            participants = (self.interaction.get_participants)(participant_type=participant_type, sim=self.context.sim, 
             target=self.target, listener_filtering_enabled=False, **self.interaction_parameters)
        else:
            if self.super_interaction is not None:
                participants = (self.super_interaction.get_participants)(participant_type=participant_type, sim=self.context.sim, 
                 target=self.target, listener_filtering_enabled=False, 
                 target_type=self.affordance.target_type, **self.interaction_parameters)
            else:
                participants = (self.affordance.get_participants)(participant_type=participant_type, sim=self.context.sim, 
                 target=self.target, carry_target=self.context.carry_target, 
                 listener_filtering_enabled=False, 
                 target_type=self.affordance.target_type, **self.interaction_parameters)
...
class DoubleObjectResolver(Resolver):
    ...
    def get_participants(self, participant_type, **kwargs):
        ...
        if participant_type == ParticipantType.ActorClanLeader:
            clan_service = services.clan_service()
            if not self._source_obj.is_sim or clan_service is None:
                return ()
        ...
        if participant_type == ParticipantType.TargetClanLeader:
            clan_service = services.clan_service()
            if not self._target_obj.is_sim or clan_service is None:
                return ()
